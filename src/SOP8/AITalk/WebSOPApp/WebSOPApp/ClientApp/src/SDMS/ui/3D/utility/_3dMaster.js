...
export class _3dMaster {
    ...
    constructor(timelog, ref3D, refBuildingGroupLabels, refBuildingLabels, refEquipZoneLabels, refFacilityTooltip, refFcltyLabels, setState, getState, contents3D) {
        this.renderer = null;
        this.scene = null;
        this.camera = null;
        this.dirLight = null;
    ...
    _init(buildingGroupList) {
        ...
        this.textPoiManager = new HtmlTextPoiManager(this.refBuildingGroupLabels, this.refBuildingLabels, this.refEquipZoneLabels, this.refFacilityTooltip, this.refFcltyLabels, this.scene, this.props.spatialManager, this);
        this.textPoiManager.initCfdLocation();
    ...
    needCameraRotation() {
        ...
        if (this.getState().useIdleTime === false)
            return false;
    ...
    onDoubleClick(event) {
        if (this.props.isEditMode) {
            if (this.textPoiManager) {
                ...
                if (this.textPoiManager.isEquipZoneText(poi)) {
                    this.textPoiManager.setEditText(poi);
                }
            }
        }
    }
    ...
    onClick(event) {
        ...
        if (this.poiManager) {
            ...
            if (this.props.isEditMode) {
                this.editModeManager.onClick(poi, this.camera, this.props.editMenu, event.nativeEvent.offsetX, event.nativeEvent.offsetY, this.props.currentModel.currentZoneNo, this.props.spatialManager, this.scene);
            }
            else {
                if (!poi) {
                    ...
                    else if (this.textPoiManager.onClickCfdText(event)) {
                        return;
                    }
                    else if (this.textPoiManager.onClickFacilityTooltip(event)) {
                        return;
                    }
                }
                ...
                if (poi) {
                    ...
                    if (this.boundingBoxModel) {
                        ...
                        this.props.onSelectFacility(this.prevIndoorFacility.object.name);
                    }
                }
            }
        }
    ...
    onMouseMove(event) {
        if (this.props.isEditMode) {
            this.editModeManager.move(event.nativeEvent.offsetX, event.nativeEvent.offsetY, this.props.editMenu);
        }
        else {
            const current = new Date();
    ...
    onCompleteLoadingCfd(frameSeconds, buildingNo, cfdLoader) {
        ...
        if (this.cfdFrameSeconds && this.cfdFrameSeconds.length > 0) {
            this.textPoiManager.setCfdBuilding(buildingNo);
    ...
    setSimulationMode(mode) {
        ...
        if (this.textPoiManager) {
            this.textPoiManager.setCfdMode(this.isSimulationMode);
        }
