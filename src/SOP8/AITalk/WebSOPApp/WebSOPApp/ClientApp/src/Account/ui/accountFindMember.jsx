...
function AccountFindMember(props) {
    ...
    const selectedArray = (() => {
        if (props.mode === 'edit') {
            // 편집은 드래프트/서버값
            return props.selectedMembersDraft ||
                    props.selectedSpread?.regularMembers ||
                    [];
        } else { // 'add'
            // 신규등록은 항상 regularMembers.checked 기준 (팝업 내 즉시 반영)
            return (props.regularMembers || []).filter(m => m.checked);
        }
    })();
    ...
    useEffect(() => {
        ...
        let ui = [];
        let index = 1;
        let allListChecked = list.length > 0;

        for (let i = 0; i < list.length; i++) {
            const member = list[i];
            const teamName = props.getTeamName(props.regularDatas, member);

            const isChecked = selectedSet.has(member.rgl_memb_sn);
                                ...
                                ? (
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => {
                                            if (props.mode === 'edit') {
                                                // 편집
                                                props.onCheckedRowDraft?.(e.target.checked, i);
                                            } else {
                                                // 신규등록
            ...
            index++;
            if (!isChecked) allListChecked = false;
        }
    ...
    const onSubmitSelectedMember = () => {
        ...
        if (props.type === 'spread') {
            if (props.mode === 'add') {
                ...
                // 편집: 이미 드래프트에 반영되어 있으므로 별도 작업 없음
            }
        }
                                        ...
                                        : (
                                            <div>
                                                <input
                                                    ...
                                                    onChange={(e) => {
                                                        if (props.mode === 'edit') {
                                                            props.onCheckedRowDraft?.(e.target.checked, -1);
                                                        } else {
                                                            props.onCheckedRow?.(e.target.checked, -1);
                                                        }
                                                    }}
                                                />
                                            </div>
