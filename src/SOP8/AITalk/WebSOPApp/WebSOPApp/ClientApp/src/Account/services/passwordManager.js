export class PasswordManager {
    static checkValidPassword(password, passwordPolicy) {
        ...
        const [success6, message6] = PasswordManager.checkAlphabet(password, passwordPolicy.needAlphabet);

        if (success6 === false) {
            return [success6, message6];
        }

        const [success7, message7] = PasswordManager.checkNumberOrCharacter(password, passwordPolicy.needNumberOrCharacter, passwordPolicy.allowCharacters);

        if (success7 === false) {
            return [success7, message7];
        }

        return [true, ""];
    }

    static checkNumberOrCharacter(password, needNumberOrCharacter, allowCharacters) {
        if (needNumberOrCharacter === false)
            return [true, ""];
        ...
        return [false, "숫자 또는 특수문자가 적어도 하나 이상 존재하여야만 합니다."];
    }

    static checkAlphabet(password, needAlphabet) {
        if (needAlphabet === false)
            return [true, ""];
        ...
        return [false, "영문자가 적어도 하나 이상 존재하여야만 합니다."];
    }
