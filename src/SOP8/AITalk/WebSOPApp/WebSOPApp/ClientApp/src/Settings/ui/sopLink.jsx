...
function SopLink(props) {
    ...
    const onClickDisasterData = (disasterCategory, subDisasterCategory, disasterData) => {
        ...
        for (const linkedSOP of newLinkedSOPs) {
            if (linkedSOP.sensorTypeName === selectedSensorType?.sensorTypeName &&
        ...
        sopData.zone_sn = selectedZone.zoneNo;
        ...
        sopData.buildingGroupName = selecteBuildingGroup.displayText;
        sopData.buildingName = selecteBuilding.displayText;
        sopData.zoneName = selectedZone.name;
    ...
    const getLinkedSopDataUI = () => {
        ...
        for (let i = 0; i < linkedSOPs.length; i++) {
            ...
            ui.push(
                <li key={`linkedSOP_${i}`}>
                    <div>{i + 1}</div> 
                    <div><span>{`${linkedSOP.buildingGroupName} > ${linkedSOP.buildingName} > ${linkedSOP.zoneName}`}</span></div>
