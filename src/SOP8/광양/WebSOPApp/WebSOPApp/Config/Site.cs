...
namespace WebSOPApp.Config
{
    public class Site : ILoginOption, IAlarmOption
    {
        ...
        private bool m_bEgovLogin = false;
        private string m_strEgovLoginUrl = "";
        ...
        public bool EgovLogin
        {
            get { return m_bEgovLogin; }
            set { m_bEgovLogin = value; }
        }

        public string EgovLoginUrl
        {
            get { return m_strEgovLoginUrl; }
            set { m_strEgovLoginUrl = value; }
        }
        ...
        public void ReadConfig(IConfiguration config, string strHeader = "Site")
        {
            ...
            ReadString(config, strHeader + ":EgovLoginURL", ref m_strEgovLoginUrl);

            string strAutoLogin = config[strHeader + ":AutoLogin"];
            string strEgovLogin = config[strHeader + ":EgovLogin"];
            string strStreamServerURL = config[strHeader + ":StreamServerURL"];
            ...
            if (strEgovLogin != null && strEgovLogin.Trim().Length > 0)
            {
                strEgovLogin = strEgovLogin.ToLower().Trim();

                if (strEgovLogin == "true")
                    EgovLogin = true;
                else if (strEgovLogin == "false")
                    EgovLogin = false;
            }

            if (strStreamServerURL != null && strStreamServerURL.Trim().Length > 0)
            {
