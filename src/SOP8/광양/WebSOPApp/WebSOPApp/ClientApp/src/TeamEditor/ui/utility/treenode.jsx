...
class TreeNode extends Component {
	...
	getMemberCount(teamData) {
		const members = this.props.members;
		const teamType = this.props.teamType;
		if (!members || !teamData) return 0;

		let count = 0;
		if (teamType === TeamEditorResource.ID.textRegular) {
			count = members.filter(m => m.rgl_sn === teamData.No).length;
		} else {
			count = members.filter(m => m.temporary?.tmpr_sn === teamData.No).length;
		}

		if (teamData.Children && teamData.Children.length > 0) {
			teamData.Children.forEach(child => {
				count += this.getMemberCount(child);
			});
		}
	...
	getDisplayCount(teamData) {
		if (!teamData) return null;

		if (this.props.members) {
			return this.getMemberCount(teamData);
		}

		if (teamData.TotalCount !== null && teamData.TotalCount !== undefined) {
			return teamData.TotalCount;
		}
	...
	render() {
		...
		if (selectedTeam !== null && selectedTeam !== undefined &&
			...
			selectedTeam.No === teamTreeData.No) {
		...
		let output = null;
		const memberCount = this.getDisplayCount(this.props.teamTreeData);
		...
		if (this.props.teamTreeData.Children === null || this.props.teamTreeData.Children === undefined || this.props.teamTreeData.Children.length === 0) {
			...
			<>
				{
					(this.props.editNodeID === this.props.teamTreeData.No || this.state.nodeEdit) ?
						...
						<a className={selectNodeClass} id={'wordIength'} onClick={(e) => this.onTreeNodeChanged(e)} onContextMenu={(e) => this.onTreeNodeChanged(e)}>
							{this.props.teamTreeData.TeamName}{memberCount !== null ? ` (${memberCount})` : ''}
				...
				<>
					<i className="fa-minus"> 더보기</i>
					{
						(this.props.editNodeID === this.props.teamTreeData.No || this.state.nodeEdit) ?
							...
							:
							<>
							<h5 className={selectNodeClass} onClick={this.onTreeNodeChanged} onContextMenu={(e) => this.onTreeNodeChanged(e)}>{this.props.teamTreeData.TeamName}{memberCount !== null ? ` (${memberCount})` : ''}
								<span className={'editArea'}>
									<span className={'treeEdit'} onClick={this.props.editTeam}></span>
									<span className={'treeMinus'} onClick={this.props.removeTeam}></span>
									<span className={'treePlus'} onClick={this.props.addTeam}></span>
