...
import { HistoryComponent, HistoryMenuComponent } from '../styled/SensorDetectHistoryStyled';
...
function History(props) {
    ...
    const [sensorTypeSubTypes, setSensorTypeSubTypes] = useState(null); // 센서타입 서브타입 바인딩 데이터
    ...
    const sensorDetectHistoryRef = useRef();
    ...
    const onClickLogo = () => {
        ...
        if (sensorDetectHistoryRef.current === null) return;
        sensorDetectHistoryRef.current.resetPage();
    }
    ...
    const initSensorTypeSubTypes = async () => {
        const result = await ExternalController.GetExternalSensorTypeSubTypes();
        
        if (result) {
            setSensorTypeSubTypes(result);
        }
    }
    ...
    return (
        <HistoryComponent>
            <HistoryMenuComponent>
                <ul className={'hslMenu'}>
                    <li className={content === HistoryResource.menu.센서_탐지_이력 ? 'on' : ''} onClick={() => changeContent(HistoryResource.menu.센서_탐지_이력)}><a>센서 탐지 이력</a></li>
                    <li className={content === HistoryResource.menu.센서_탐지_분석 ? 'on' : ''} onClick={() => changeContent(HistoryResource.menu.센서_탐지_분석)}><a>센서 탐지 분석</a></li>
                    <li className={content === HistoryResource.menu.SOP_이력 ? 'on' : ''} onClick={() => changeContent(HistoryResource.menu.SOP_이력)}><a>SOP 이력</a></li>
                </ul> 
            </HistoryMenuComponent>
        {
            content === HistoryResource.menu.센서_탐지_이력 &&
                <SensorDetectHistory key='history_SensorDetectHistory' changeContent={changeContent} useSensorTypes={useSensorTypes} selectedSiteNo={selectedSiteNo} lastClickRow={lastClickRow} setLastClickRow={setLastClickRow} showConfirmDialog={showConfirmDialog} onCloseConfirmDialog={onCloseConfirmDialog} sensorTypes={sensorTypes} sensorList={sensorList} formatIsoToDateTime={formatIsoToDateTime} ref={sensorDetectHistoryRef} handleToast={handleToast} />
        }
        {
            content === HistoryResource.menu.센서_탐지_분석 &&
                <SensorDetectAnalysis key='history_SensorDetectAnalysis' changeContent={changeContent} useSensorTypes={useSensorTypes} selectedSiteNo={selectedSiteNo} lastClickRow={lastClickRow} setLastClickRow={setLastClickRow} showConfirmDialog={showConfirmDialog} onCloseConfirmDialog={onCloseConfirmDialog} sensorTypes={sensorTypes} sensorList={sensorList} formatIsoToDateTime={formatIsoToDateTime} />
        }
        {
        ...
        {/* {
            content === HistoryResource.menu.보고서 &&
        ...
        {
            content === HistoryResource.menu.통계 &&
