...
export default class ProjectResource {
    ...
    static async clearLoginUser() {
        window.localStorage.removeItem(SessionString.Key.account + "_" + ProjectResource.site_sn.toString());
    }
    ...
    static applyClientConfig(config) {
        if (!config) {
            return;
        }

        ProjectResource.egovLogin = config.egovLogin === true;
        ProjectResource.egovLoginUrl = ProjectResource.normalizeBaseUrl(config.egovLoginUrl);
    }
    ...
    static getAccountBaseUrl() {
        if (ProjectResource.egovLogin === true && ProjectResource.egovLoginUrl.length > 0) {
            return ProjectResource.egovLoginUrl;
        }

        return ProjectResource.normalizeBaseUrl(ProjectResource.baseUrl);
    }
