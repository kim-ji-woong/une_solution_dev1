...
function Event(props) {
    ...
    useEffect(() => {
                ...
                for (const alarm of displayAlarmList) {
                    ...
                    ui.push(
                        ...
                        >
                                ...
                                <div className='eventIconWrap'>
                                    ...
                                    {hasSpread &&
                                        <IconButton
                                            variant="unfill"
                                            size="xxs"
                                            icon={<Icon.IconAlarmStatus />}
                                            onClick={(e) =>  onClickNotifyAlarm(e, alarm)}
                                        >
                                            재난신고
                                        </IconButton>
                                    }
                            ...
                            <footer>
                                <TextButton 
                                    variant="unfill" size="xs"
                                    onClick={(e) => onClickAlarmSop(alarm)}
                                    rightIcon={<Icon.Arrow direction={"right"} />}
                                >
                                    SOP 바로가기
                                </TextButton>
                            </footer>
        ...
        renderUI();
    }, [displayAlarmList, props.sensorAlarm, props.sensorList, props.selectedAlarm, props.selectedSensor, memoAlarm]);
    ...
    const onClickClearAlarm = async (e, allClear, alarm) => {
        ...
        if (allClear) {
            ...
            props.showConfirmDialog(ProjectResource.dialogTypes.MALFUNCTION, ["이벤트 알람을 선택 종료하시겠습니까?", "종료된 이벤트는 되돌릴 수 없으며, 이력 관리 화면에서 확인 가능합니다."], ["취소", "종료하기"], clearAlarm);
        }
    } 
    ...
    const onClickSaveMemo = async (memo) => {
        ...
        if (success) {
            ...
            updateAlarmMemo(alarm.sensorZoneHistoryNo, memo);
            closeMemoPopup();
        }
        else {
            props.showConfirmDialog(ProjectResource.dialogTypes.ERROR, [message], null, null);
    ...
    const updateAlarmMemo = (sensorZoneHistoryNo, memo) => {
        if (!props.setSensorAlarms) return;

        props.setSensorAlarms(prev =>
            prev.map(alarm =>
                alarm.sensorZoneHistoryNo === sensorZoneHistoryNo
                    ? { ...alarm, memo }
                    : alarm
            )
        );
    ...
    const onClickNotifyAlarm = (e, alarm) => {
        ...
        selectedAlarmRef.current = alarm; // ref에 선택된 alarm 저장
        props.showConfirmDialog(ProjectResource.dialogTypes.QUESTION, ["실제 재난 신고를 진행하시겠습니까?", "설정한 전파 대상자에게 문자가 전송됩니다."], ["취소", "진행하기"], doNotifyAlarm);
    }

    const doNotifyAlarm = async (index) => {
        if (index === 0) {
            props.onCloseConfirmDialog();
            return;
        }
        ...
        if (result) {
            props.handleToast("실제 재난 신고가 완료되었습니다");
        }
        else if (result === null) {
            props.showConfirmDialog(ProjectResource.dialogTypes.ERROR, [message], null, null);
        }
                ...
                <div className={'content'}>
                    <div className='sortWrap'>
                        <DropBox
                            ...
                            options={[
                                { value: 'all', label: '전체' },
                                { value: 'pending', label: '미대응' },
                                { value: 'responded', label: '대응' },
                            ]}
                            openId={openDropId}
