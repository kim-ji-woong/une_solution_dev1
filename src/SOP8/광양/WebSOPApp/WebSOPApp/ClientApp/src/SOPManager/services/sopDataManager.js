...
export default class SopDataManager {
	...
	static copySectionData(src, trg) {
		...
		if (src.component.compn_code === SectionData.AnnotationType) {
			SectionDataAnnotation.copyTo(src, trg);
		}
		else if (src.component.compn_code === SectionData.DecisionType) {
			SectionDataDecision.copyTo(src, trg);
		}
		else if (src.component.compn_code === SectionData.EndpointType) {
			SectionDataEndpoint.copyTo(src, trg);
		}
		else if (src.component.compn_code === SectionData.InternalType) {
			SectionDataInternal.copyTo(src, trg);
		}
		else if (src.component.compn_code === SectionData.ProcessType) {
	...
	static sectionComponenttoSectionData(component, sectionNumber) {
		...
		if (component.compn_code === SectionData.ProcessType) {
			sectionData = SectionDataProcess.componentToSectionData(component);
		}
		else if (component.compn_code === SectionData.AnnotationType) {
			sectionData = SectionDataAnnotation.componentToSectionData(component);
		}
		else if (component.compn_code === SectionData.DecisionType) {
			sectionData = SectionDataDecision.componentToSectionData(component);
		}
		else if (component.compn_code === SectionData.EndpointType) {
			sectionData = SectionDataEndpoint.componentToSectionData(component);
		}
		else if (component.compn_code === SectionData.InternalType) {
	...
	static jsonToSection(rawSection) {
		...
		if (rawSection.compn_code === SectionData.ProcessType) {
			sectionData = SectionDataProcess.fromJson(rawSection);
		}
		else if (rawSection.compn_code === SectionData.InternalType) {
			sectionData = SectionDataInternal.fromJson(rawSection);
		}
		else if (rawSection.compn_code === SectionData.EndpointType) {
			sectionData = SectionDataEndpoint.fromJson(rawSection);
		}
		else if (rawSection.compn_code === SectionData.DecisionType) {
			sectionData = SectionDataDecision.fromJson(rawSection);
		}
		else if (rawSection.compn_code === SectionData.AnnotationType) {
	...
	static sectionToJson(section) {
		...
		if (section.component.compn_code === SectionData.ProcessType) {
			return SectionDataProcess.toJson(section);
		}
		else if (section.component.compn_code === SectionData.InternalType) {
			return SectionDataInternal.toJson(section);
		}
		else if (section.component.compn_code === SectionData.EndpointType) {
			return SectionDataEndpoint.toJson(section);
		}
		else if (section.component.compn_code === SectionData.DecisionType) {
			return SectionDataDecision.toJson(section);
		}
		else if (section.component.compn_code === SectionData.AnnotationType) {
	...
	static setArrowPosition(arrow, stepMemberData, isBegin, columnIndex, rowIndex, position, text) {
		if (isBegin) {
			...
			arrow.endCell = null;
			arrow.RowIndex = rowIndex;
			arrow.ColumnIndex = columnIndex;
					...
					for (let j = 0; j < stepMemberCount; j++) {
						...
						if (stepMember.sections) {
							...
							for (let k = 0; k < sectionCount; k++) {
								...
								if (section.component.compn_code === SectionData.ProcessType ||
									section.component.compn_code === SectionData.InternalType) {
