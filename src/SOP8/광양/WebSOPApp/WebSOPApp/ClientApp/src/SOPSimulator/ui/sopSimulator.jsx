...
function SopSimulator(props) {
    ...
    useEffect(() => {
        if (isFirstRef.current && store.getState().sopHistory && sopDatas.length === 0) {
            loadSOP(store.getState(), currentSopTabKeyRef.current, currentActionStepRef.current);
        }
    }, [])
    ...
    const loadSOP = async (storeValue, sopTabKey, actionStep) => {
        ...
        if (selectedSiteNo === null) {            
            if (userInfo) {
                ...
                // 현재 -1은 지원하지 않아 무조건 siteNo를 보내도록 수정 (한봄희 260119)
        ...
        if (sopHistories === null || sopHistories.length === 0) {
            ...
            for (let i = 0; i < sopHistories.length; i++) {
                ...
                for (let j = 0; j < sopHistory.actionStepDatas.length; j++) {
                    const actionStepData = sopHistory.actionStepDatas[j];

                    if (actionStepData.confirmTimeoutCloseSOP && actionStepData.actionStepHistory) {
                        closeSOPs.push(actionStepData.actionStepHistory.action_step_hist_sn);
                        break;
                    }
                }
            }
            ...
            if (closeSOPs.length > 0) {
                ...
                for (let i = 0; i < historyLength; i++) {
                    ...
                    for (let j = 0; j < sopHistory.actionStepDatas.length; j++) {
                        ...
                        if (actionStepData.actionStepHistory && actionStepData.actionStepHistory.action_step_hist_sn === closeSOPHistoryID) {
                            sopTabIndex = i;
                            currentSopData = sopHistory;
                            newCurrentActionStep = actionStepData;
                            confirmDialogData = actionStepData.actionStepHistory;
                            break;
                        }
                    }
                ...
                if (confirmDialogData) {
                    ...
                    if (confirmDialogData.sensor_zone_hist_sn && confirmDialogData.sensor_zone_hist_sn > 0) {
        ...
        for (const actionStep of actionStepDatas) {
            if (actionStep.componentHistories) {
                ...
                for (const history of actionStep.componentHistories) {
                    ...
                    if (SectionData.isProcessType(history.compn_code)) {
                        updateProcessSection(section, history.componentHistoryDetails[0]);
                    } else if (SectionData.isTransmissionType(history.compn_code)) {
    ...
    const confirmDialogCloseSOP = async (index) => {        
        const actionStepHistory = confirmDialogData.current;
    ...
    const onClickLogo = () => {
        onSelectMenu(SopSimulatorResource.menu.SOP_불러오기, null);
    }
