...
namespace Base.TeamEditor.BLL.Excel.Writer
{
    using Reader;

    class RegularTeamWriter : ExcelWriter
    {
        private const string Column_TeamName = "부서";
        private const string Column_MemberName = "이름";
        private const string Column_MemberID = "사번";
        ...
        public RegularTeamWriter(IDataManager dataManager, int? siteNo)
        {
        ...
        {
            if (index == 0)
                return RegularTeamWriter.Column_TeamName;
            else if (index == 1)
                return RegularTeamWriter.Column_MemberName;
            else if (index == 2)
                return RegularTeamWriter.Column_MemberID;
            else if (index == 3)
                return RegularTeamWriter.Column_PhoneNumber;
            else if (index == 4)
                return RegularTeamWriter.Column_OfficePhoneNumber;
            else if (index == 5)
                return RegularTeamWriter.Column_JobLevel;
            else if (index == 6)
                return RegularTeamWriter.Column_JobPosition;
            else if (index == 7)
                return RegularTeamWriter.Column_Email;
        ...
        protected override ICollection<SheetData> ReadSheetDatas(HSSFWorkbook workbook, out string strErrorMessage)
        {
            ...
            Dictionary<int, Regular> dicRegulars = RegularMemberReader.GetRegularTeams(m_dataManager, out strErrorMessage);
        ...
        public static Dictionary<int, Option> ReadTeamOptions(IDataManager dataManager, out string strErrorMessage)
        {
        ...
        public static Dictionary<Regular, List<RegularMember>> ReadRegularMembers(IDataManager dataManager, int? siteNo, out string strErrorMessage)
        {
        ...
        private SheetData MakeSheet(HSSFWorkbook workbook, Dictionary<Regular, List<RegularMember>> dicRegularMembers, Dictionary<int, Option> dicTeamOptions, Dictionary<int, Regular> dicRegulars)
        {
            ...
            foreach (var pair in dicRegularMembers)
            {
                ...
                foreach (RegularMember regularMember in pair.Value)
                {
                    ...
                    sheetData.ColumnDatas[0].Add(GetText(RegularMemberReader.GetTeamPath(regular, dicRegulars)));
                    sheetData.ColumnDatas[1].Add(GetText(regularMember.memb_name));
                    sheetData.ColumnDatas[2].Add(GetText(regularMember.unq_key));
                    sheetData.ColumnDatas[3].Add(GetText(regularMember.telno));
                    sheetData.ColumnDatas[4].Add(GetText(regularMember.offm_telno));
                    sheetData.ColumnDatas[5].Add(GetText(strJobLevel));
                    sheetData.ColumnDatas[6].Add(GetText(strJobPosition));
                    sheetData.ColumnDatas[7].Add(GetText(regularMember.email));

                    /*for (int i=0;i<GetColumnCount();i++)
                    {
                        SetBodyStyle(workbook, sheetData, dicStyles, index, memberCount, i);
                    }

                    sheetData.RowHeight[index] = RowHeight;*/
                    index++;
                }
            }

            // 팀원이 한명도 없는 팀도 엑셀파일에 담는다.
        ...
        private int MakeEmptyTeamSheet(Dictionary<int, Regular> dicUsedTeams, Dictionary<int, Regular> dicRegulars, SheetData sheetData)
        {
            ...
            foreach (Regular regular in unusedTeams)
            {
                ...
                sheetData.ColumnDatas[7].Add(GetText(""));
            }
        ...
        protected override int WritePrev(ISheet sheet, HSSFWorkbook workbook)
        {
            ...
            sheet.SetColumnWidth(2, GetColumnWidth(dPixelWidth, dWidth, 125));
            sheet.SetColumnWidth(3, GetColumnWidth(dPixelWidth, dWidth, 130));
            sheet.SetColumnWidth(4, GetColumnWidth(dPixelWidth, dWidth, 150));
            sheet.SetColumnWidth(5, GetColumnWidth(dPixelWidth, dWidth, 64));
            sheet.SetColumnWidth(6, GetColumnWidth(dPixelWidth, dWidth, 64));
            sheet.SetColumnWidth(7, GetColumnWidth(dPixelWidth, dWidth, 150));
