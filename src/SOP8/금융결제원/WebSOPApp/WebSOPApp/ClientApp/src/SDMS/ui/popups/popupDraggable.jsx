...
class PopupDraggable extends Component {
    ...
    initPopupState() {
        ...
        // 특정 팝업만 중앙 배치
        else if (this.props.useInitialCenter === true) {
            this.setInitialCenterPosition(popup);
        }
    }
    ...
    // 논모달 좌측 중앙에 배치 (left만 중앙)
    setInitialCenterPosition(popup) {
        if (!popup) return;

        const screenW = document.body.clientWidth;
        const popupW = popup.offsetWidth;

        const leftPx = (screenW - popupW) / 2;
