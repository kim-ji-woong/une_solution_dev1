...
export class GlbLoader {
    ...
    _loadRootModel(contents, nextIndex, files, mode, visible, buildingGroupList, gltfOptions) {
        ...
        loader.load(fileName, function (object) {
            ...
            if (mode === _3dMaster.Mode_Outdoor_All || mode === _3dMaster.Mode_Outdoor_Part) {
                ...
                const boundingBoxes = [];
                _3dMaster.hideBoundingBoxes(modelNode, boundingBoxes);

                if (boundingBoxes.length > 0) {
                    _this._3dMaster.buildingBoundingBox = boundingBoxes[0];
                }

                // 침입센서 영역 하이드
                _3dMaster.hideIntrusionArea(modelNode, false, _this._3dMaster.intrusionAreas);
            }
    ...
    loadFile(contents, visible, cameraOptions, mode, gltfOptions, postMethod = null, postMethodParam = null) {
        ...
        loader.load(fileName, function (object) {
            ...
            console.log('BoundingBox min:' + box.min.x + ", " + box.min.y + ", " + box.min.z);
            console.log('BoundingBox max:' + box.max.x + ", " + box.max.y + ", " + box.max.z);*/
    ...
    _addIndoorModel(modelNode, mode, object, gltfOptions, contentsFile, postMethod, postMethodParam, cameraOptions) {
        ...
        if (mode === _3dMaster.Mode_Outdoor_Part/*visible*/) {
            ...
            // 침입센서 영역 하이드
            _3dMaster.hideIntrusionArea(modelNode, false, this._3dMaster.intrusionAreas);
        }
