...
import { PoiManager } from "../ui/3D/poi/poiManager";
...
export class SpatialManager {
    ...
    make3dModels(siteBuildingGroupList, siteModels, sensorList, tpsList) {
                    ...
                    for (const buildingGroup of siteBuildingGroup.buildingGroups) {
                        ...
                        for (const building of buildingGroup.buildingDatas) {
                            ...
                            for (const zone of building.zoneDatas) {
                                ...
                                this.makeupTpsZone(zone, tpsList);
    ...
    makeupTpsZone = (zoneData, equipmentZoneData) => {

        for (const equipmentZone of zoneData?.equipmentZoneDatas) {
            if (equipmentZoneData?.find(x => x.tpsNo === equipmentZone.equipZoneNo)) {
                if (!equipmentZone.userData) {
                    equipmentZone.userData = {};
                }                
                equipmentZone.userData.isTPS = true;
            }
        }
    }
            ...
            for (const sensorType of sensorTypes) {
                if (sensorType.sensors) {
                    for (const sensor of sensorType.sensors) {
                        if (sensor.sensor) {
                            const prevSensor = this.sensors[sensor.sensor.sensor_sn];

                            if (prevSensor && (prevSensor.isOpened || prevSensor.isOpened === false)) {
                                sensor.sensor.isOpened = prevSensor.isOpened;
                            }
    ...
    // 바뀌었는가?
    checkDoorStatus(doorStatusList, zoneNo) {
        if (this.prevDoorStatus.zoneNo !== zoneNo) {
            return true;
        }

        const prevCount = this.prevDoorStatus.doorStatusList.length;
        const currentCount = doorStatusList.length;
        ...
        for (let i = 0; i < currentCount; i++) {
            const prevStatus = this.prevDoorStatus.doorStatusList[i];
            const currentStatus = doorStatusList[i];

            if (prevStatus.isOpened !== currentStatus.isOpened ||
                prevStatus.sensorNo !== currentStatus.sensorNo) {
                return true;
            }
        }
    ...
    updateDoorStatus(doorStatusList, zoneNo, poiManager) {
        const isChanged = this.checkDoorStatus(doorStatusList, zoneNo);

        if (isChanged === false) {
            return;
        }

        this.prevDoorStatus.zoneNo = zoneNo;
        this.prevDoorStatus.doorStatusList = doorStatusList;

        for (const status of doorStatusList) {
            const sensor = this.getZoneSensor(zoneNo, status.sensorNo, SdmsResource.facilityType.DOOR);

            if (sensor) {
                sensor.isOpened = status.isOpened ? true : false;

                const poi = poiManager.getSensorPOI(zoneNo, sensor.sensor_sn, sensor.sensor_ty_code, sensor.subTypeNo);

                if (poi) {
                    let doorPointType = PoiManager.DoorPointType.none;

                    if (poi.material.name.endsWith("point")) {
                        doorPointType = PoiManager.DoorPointType.point;
                    }
                    else if (poi.material.name.endsWith("important")) {
                        doorPointType = PoiManager.DoorPointType.important;
                    }

                    poiManager.changeDoorPoi(poi, sensor.isOpened, doorPointType);
                }
            }
        }
    }
