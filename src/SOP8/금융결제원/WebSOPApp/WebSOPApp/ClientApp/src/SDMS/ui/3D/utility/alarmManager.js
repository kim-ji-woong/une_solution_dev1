...
export class AlarmManager {
    ...
    /// 알람의 zoneNo를 POI 키에 쓰인 센서의 실제 zone_sn으로 정규화한다.
    /// 외부 알람은 alarm.zoneNo가 sentinel 20000이지만 POI 키는 센서의 real zone_sn으로 생성되므로
    /// getSensorPOI 호출 시 반드시 이 메서드로 정규화해야 한다. 실내는 값이 동일해 무변화.
    _getPoiZoneNo(alarm) {
        const sensor = this.spatialManager.getSensor(alarm.sensorNo);
        return sensor?.zone_sn ?? alarm.zoneNo;
    }
        ...
        if (alarm) {
            ...
            if (file) {
                if (modelNode) {
                    ...
                    else if (this.spatialManager.isSameZone(this._3dMaster.props.currentModel?.currentZoneNo, alarm.zoneNo)) {
                        // 현재 모델에서 새로운 알람이 발생했다면... (외부: null===20000 방지를 위해 isSameZone 사용)
                        const poi = this._3dMaster.poiManager.getSensorPOI(this._getPoiZoneNo(alarm), alarm.sensorNo, alarm.facilityType, alarm.sensorSubType);
    ...
    static _showAlarm(params) {
        ...
        for (const alarm of alarms) {
            // 외부 알람은 alarm.zoneNo가 sentinel 20000이므로 real zone_sn으로 정규화
            const poi = _this._3dMaster.poiManager.getSensorPOI(_this._getPoiZoneNo(alarm), alarm.sensorNo, alarm.facilityType, alarm.sensorSubType);
    ...
    _hideAlarm(alarm) {
        if (alarm) {
            // 외부 알람은 alarm.zoneNo가 sentinel 20000이므로 real zone_sn으로 정규화해야 POI를 찾을 수 있다
            const poi = this._3dMaster.poiManager.getSensorPOI(this._getPoiZoneNo(alarm), alarm.sensorNo, alarm.facilityType, alarm.sensorSubType);
    ...
    moveToAlarm(alarm, sensorAlarms) {
        if (alarm) {
            // 외부에서는 currentZoneNo=null, alarm.zoneNo=20000으로 !== 가 항상 true가 되어 불필요한 재이동이 발생하므로 isSameZone으로 비교
            if (!this.spatialManager.isSameZone(this._3dMaster.props.currentModel?.currentZoneNo, alarm.zoneNo)) {
    ...
    getAlarmModel(alarm) {
        ...
        if (zone) {
            // 1. 실내 zone: zone 자체에 등록된 모델 사용
            const model = zone.model;

            if (model?.file) {
                return [model.file, zone.modelNode];
            }
            else if (zone.buildingNo === null) {
                // 2. 외부(실외) zone: 모델이 zone이 아닌 site(외부영역 모델)에 등록됨
                const site = this.spatialManager.getSite(zone.siteNo);

                if (site?.model?.file) {
                    return [site.model.file, site.modelNode];
                }
            }
        }
