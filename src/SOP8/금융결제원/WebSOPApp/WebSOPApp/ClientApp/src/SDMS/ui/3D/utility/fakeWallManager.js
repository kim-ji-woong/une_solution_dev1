...
export class FakeWallManager {
    ...
    changeMode(mode) {
        if (mode === SdmsResource.ID.poi_editSubMenu.add_wall) {
            this.mode = FakeWallManager.Mode_Add_NoClick;
        }
        else if (mode === SdmsResource.ID.poi_editSubMenu.move_wall) {
            this.mode = FakeWallManager.Mode_Move;
        }
        else if (mode === SdmsResource.ID.poi_editSubMenu.length_wall) {
            this.mode = FakeWallManager.Mode_Resize;
        }
        else if (mode === SdmsResource.ID.poi_editSubMenu.rotate_wall) {
            this.mode = FakeWallManager.Mode_Rotate;
        }
        else if (mode === SdmsResource.ID.poi_editSubMenu.delete_wall) {
    ...
    onClick(x, y, zoneNo, camera, spatialManager, scene) {
        ...
        if (this.mode === FakeWallManager.Mode_Add_NoClick) {
            ...
            if (this.currentWall === null) {
                ...
                if (vCurrent) {
                    this.setOriginState(this.currentWall);
    ...
    setOriginState(wall) {
        this.originState.rotation = new THREE.Vector3(wall.rotation.x, wall.rotation.y, wall.rotation.z);
        this.originState.position = new THREE.Vector3(wall.position.x, wall.position.y, wall.position.z);
        this.originState.scale = new THREE.Vector3(wall.scale.x, wall.scale.y, wall.scale.z);
    }

    rollback() {
        if (this.currentWall && this.originState.rotation) {
            this.currentWall.rotation.set(this.originState.rotation.x, this.originState.rotation.y, this.originState.rotation.z);
            this.currentWall.position.set(this.originState.position.x, this.originState.position.y, this.originState.position.z);
            this.currentWall.scale.set(this.originState.scale.x, this.originState.scale.y, this.originState.scale.z);
        }
    }
    ...
    stop() {
        ...
        else if (this.mode === FakeWallManager.Mode_Move && this.currentWall) {
            this.currentWall.position.set(this.vOrigin.x, this.vOrigin.y, this.vOrigin.z);
            this.initNoneValue();
        }
        else if (this.mode === FakeWallManager.Mode_Resize && this.currentWall) {
            this.rollback();
            this.initNoneValue();
        }
        else if (this.mode === FakeWallManager.Mode_Rotate && this.currentWall) {
            this.rollback();
            this.initNoneValue();
        }
        else {
            this.editModeManager.changeMode(SdmsResource.ID.menu.editMode_fakeWall, SdmsResource.ID.poi_editSubMenu.none);
            this.mode = FakeWallManager.Mode_None;
        }
    }
    ...
    deleteFakeWall(fakeWall, zoneDatas) {
        ...
        this.editModeManager.showMessageBox(EditModeManager.msg_info, "선택한 가벽이 삭제되었습니다.");
    }
