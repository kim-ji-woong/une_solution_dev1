...
export class SDMSController {
    static alarmTimerCheck = false;
    static alarmTimerId = null;

    // 출입문 관련
    static doorTimerCheck = false;
    static doorTimerId = null;
    static totalDoorStatusTimerCheck = false;
    static totalDoorStatusTimerId = null;

    // 출입자 관련
    static comingHistoryTimerCheck = false;
    static comingHistoryTimerId = null;
    static areaComingHistoryTimerCheck = false;
    static areaComingHistoryTimerId = null;
    static lastComingPersonTimerCheck = false;
    static lastComingPersonTimerId = null;

    // 출입차량 관련
    static parkingInfoTimerCheck = false;
    static parkingInfoTimerId = null;

    static StartWatchAlarmTimer() {
        this.stopWatchAlarmTimer();        
        ...
        const alarmTick = async () => {
            ...
            try {
                ...
                if (this.alarmTimerCheck) {
                    this.alarmTimerId = setTimeout(alarmTick, 1500);
                }
            }
        };
    ...
    // 출입문 현황 타이머
    static StartWatchDoorTimer(zoneNo) {
        this.stopWatchDoorTimer();

        this.doorTimerCheck = true;

        const doorTick = async () => {
            if (!this.doorTimerCheck) return;

            try {
                await SDMSController.WatchDoorStatus(zoneNo);
            } catch (e) {
                console.log("Door Timer Error:", e);
            } finally {
                if (this.doorTimerCheck) {
                    this.doorTimerId = setTimeout(doorTick, 1500);
                }
            }
        };
    ...
    static stopWatchDoorTimer() {
        this.doorTimerCheck = false;

        if (this.doorTimerId) {
            clearTimeout(this.doorTimerId);
            this.doorTimerId = null;
        }
    }

    static async WatchDoorStatus(zoneNo) {
        try {
            let [doorDatas, message] = await SDMSController.requestDoorStatus(zoneNo);
            ...
            const current = store.getState().doorStatus || null;
    
            const isChanged = !isEqual(current, doorDatas);

            if (isChanged) {
                store.dispatch({
                    type: 'DOOR_STATUS',
                    doorStatus: doorDatas
                });
            }
        } catch (e) {
            console.log("WatchDoorStatus Error:", e);
        }
    }

    // 미개방 출입문 현황 타이머
    static StartWatchTotalDoorStatusTimer() {
        this.stopWatchTotalDoorStatusTimer();

        this.totalDoorStatusTimerCheck = true;

        const doorStatusTick = async () => {
            if (!this.totalDoorStatusTimerCheck) return;

            try {
                await SDMSController.WatchTotalDoorStatus();
            } catch (e) {
                console.log("Door Timer Error:", e);
            } finally {
                if (this.totalDoorStatusTimerCheck) {
                    this.totalDoorStatusTimerId = setTimeout(doorStatusTick, 1000);
                }
            }
        };
    ...
    static stopWatchTotalDoorStatusTimer() {
        this.totalDoorStatusTimerCheck = false;

        if (this.totalDoorStatusTimerId) {
            clearTimeout(this.totalDoorStatusTimerId);
            this.totalDoorStatusTimerId = null;
        }
    }

    static async WatchTotalDoorStatus() {
        try {
            let [totalCloseDoorCount, zones, message] =
                await SDMSController.requestTotalDoorStatus();
            ...
            console.log("WatchTotalDoorStatus Error:", e);
        }
    }

    // 출입자 현황 타이머 : 건물 내부
    static StartWatchComingHistoryTimer() {
        this.stopWatchComingHistoryTimer();

        this.comingHistoryTimerCheck = true;

        const comingHistoryTick = async () => {
            if (!this.comingHistoryTimerCheck) return;

            try {
                await SDMSController.WatchComingHistory();
            } catch (e) {
                console.log("ComingHistory Timer Error:", e);
            } finally {
                if (this.comingHistoryTimerCheck) {
                    this.comingHistoryTimerId = setTimeout(comingHistoryTick, 500);
                }
            }
        };
    ...
    static stopWatchComingHistoryTimer() {
        this.comingHistoryTimerCheck = false;

        if (this.comingHistoryTimerId) {
            clearTimeout(this.comingHistoryTimerId);
            this.comingHistoryTimerId = null;
        }
    }

    static async WatchComingHistory() {
        try {
            let [result, message] = await SDMSController.requestComingHistory();
            ...
            store.dispatch({
                type: 'COMING_HISTORY',
                comingHistory: result
            });
        } catch (e) {
            console.log("watchComingHistory Error:", e);
        }
    }

    // 출입자 현황 타이머 : 중요구역
    static StartWatchAreaComingHistoryTimer() {
        this.stopWatchAreaComingHistoryTimer();

        this.areaComingHistoryTimerCheck = true;

        const areaComingHistoryTick = async () => {
            if (!this.areaComingHistoryTimerCheck) return;

            try {
                await SDMSController.WatchAreaComingHistory();
            } catch (e) {
                console.log("areaComingHistory Timer Error:", e);
            } finally {
                if (this.areaComingHistoryTimerCheck) {
                    this.areaComingHistoryTimerId = setTimeout(areaComingHistoryTick, 500);
                }
            }
        };

        areaComingHistoryTick();
    }

    static stopWatchAreaComingHistoryTimer() {
        this.areaComingHistoryTimerCheck = false;

        if (this.areaComingHistoryTimerId) {
            clearTimeout(this.areaComingHistoryTimerId);
            this.areaComingHistoryTimerId = null;
        }
    }

    static async WatchAreaComingHistory() {
        try {
            let [result, message] = await SDMSController.requestAreaComingHistory();
            ...
            store.dispatch({
                type: 'AREA_COMING_HISTORY',
                areaComingHistory: result
            });
        } catch (e) {
            console.log("watchAreaComingHistory Error:", e);
        }
    }

    // 출입자 현황 타이머 : 마지막 출입자 정보 (화면 축소)
    static StartWatchLastComingPersonTimer() {
        this.stopWatchLastComingPersonTimer();

        this.lastComingPersonTimerCheck = true;

        const lastComingPersonTick = async () => {
            if (!this.lastComingPersonTimerCheck) return;

            try {
                await SDMSController.WatchLastComingPerson();
            } catch (e) {
                console.log("lastComingPerson Timer Error:", e);
            } finally {
                if (this.lastComingPersonTimerCheck) {
                    this.lastComingPersonTimerId = setTimeout(lastComingPersonTick, 500);
                }
            }
        };

        lastComingPersonTick();
    }

    static stopWatchLastComingPersonTimer() {
        this.lastComingPersonTimerCheck = false;

        if (this.lastComingPersonTimerId) {
            clearTimeout(this.lastComingPersonTimerId);
            this.lastComingPersonTimerId = null;
        }
    }

    static async WatchLastComingPerson() {
        try {
            let [result, message] = await SDMSController.requestLastComingPerson();
            ...
            store.dispatch({
                type: 'LAST_COMING_PERSON',
                lastComingPerson: result
            });
        } catch (e) {
            console.log("watchLastComingPerson Error:", e);
        }
    }

    // 출입차량 현황 타이머
    static StartWatchParkingInfoTimer() {
        this.stopWatchParkingInfoTimer();

        this.parkingInfoTimerCheck = true;

        const parkingInfoTick = async () => {
            if (!this.parkingInfoTimerCheck) return;

            try {
                await SDMSController.WatchParkingInfo();
            } catch (e) {
                console.log("ParkingInfo Timer Error:", e);
            } finally {
                if (this.parkingInfoTimerCheck) {
                    this.parkingInfoTimerId = setTimeout(parkingInfoTick, 1000);
                }
            }
        };
    ...
    static stopWatchParkingInfoTimer() {
        this.parkingInfoTimerCheck = false;

        if (this.parkingInfoTimerId) {
            clearTimeout(this.parkingInfoTimerId);
            this.parkingInfoTimerId = null;
        }
    }

    static async WatchParkingInfo() {
        try {
            let [result, message] = await SDMSController.requestCurrentParkingInfo();
            ...
            console.log("WatchParkingInfo Error:", e);
        }
    }
    ...
    // 1. 검색대상은 sensorTypes에 한정한다.
    //    sensorTypes가 null이거나 비어있으면 전체 센서타입을 대상으로 한다.
    // 2. sensorNos가 존재하면 sensorNos만 검색한다.
    // 3. sensorNos가 null이거나 비어있으면 zoneNo에 대한 센서를 검색한다.
    // 4. zoneNo도 null이고 sensorNos도 null이거나 비어있으면 전체 센서를 검색한다.
    static async requestZoneSensorList(sensorTypes = null, zoneNo = null, sensorNos = null) {
        try {
            const jsonData = SdmsJsonManager.makeRequestZoneSensorList(sensorTypes, zoneNo, sensorNos);

            const res = await fetch(ProjectResource.baseUrl + '/api/SDMS/RequestZoneSensorList', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return [null, 0, "requestZoneSensorList 실패"];
    }

    static async requestAdditableSensors(sensorType = null, siteNo = null) {
        try {
            const jsonData = SdmsJsonManager.makeRequestAdditableSensors(sensorType, siteNo);

            const res = await fetch(ProjectResource.baseUrl + '/api/SDMS/RequestAdditableSensors', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return [null, "requestAdditableSensors 실패"];
    }
    ...
    static async requesAllCCTVs() {
        try {
            const jsonData = SdmsJsonManager.makeRequesAllCCTVs();

            const res = await fetch('SDMS/SDMS/RequesAllCCTVs', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
    ...
    /*static async requestFacilityList(zoneNo = null, siteNo = null) {
        try {
            const jsonData = SdmsJsonManager.makeRequestFacilityList(zoneNo, siteNo);

            const res = await fetch('SDMS/SDMS/RequesFacilityList', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return [false, "requestFacilityList 호출에 실패하였습니다.", null];
    }*/
    ...
    static async requestSensorServerInfo(siteNo = null, sensorTypeCode = null, sensorSubType = null) {
        try {
            const jsonData = SdmsJsonManager.makeRequestSensorServerInfo(siteNo, sensorTypeCode, sensorSubType);

            const res = await fetch('api/SDMS/RequestSensorServerInfo', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
            ...
            if (res.ok) {
                ...
                if (result.success) {
                    return [result.servers, result.message];
                }
                else {
                    return [null, result.message];
                }
        ...
        return [null, "requestSensorServerInfo 실패"];
    }
    ...
    static async requestSensorInfo(sensorNo) {
        try {
            const jsonData = SdmsJsonManager.makeRequesSensorInfo(sensorNo);

            const res = await fetch('SDMS/SDMS/RequestSensorInfo', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return [null, "requestSensorInfo 실패"];
    }
    
    static async requestUpdateSensorZoneCCTVs(updateDatas, deleteDatas) {
        try {
            const jsonData = SdmsJsonManager.makeRequesUpdateSensorZoneCCTVs(updateDatas, deleteDatas);

            const res = await fetch('api/SDMS/UpdateSensorZoneCCTVs', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return [false, "requestUpdateSensorZoneCCTVs 실패"];
    }
    
    static async requestTodayWorkList() {
        try {
            const res = await fetch('SDMS/Wish/RequestTodayWorkList', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
            ...
            if (res.ok) {
                const result = await res.json();
                return [result.todayWorkList, result.success, result.message];
            }
        }
        catch (e) {
            console.log(e);
            return [null, false, "requestTodayWorkList 실패"];
        }
    }
    
    static async requestCurrentWorkPermitData() {
        try {
            const res = await fetch('SDMS/Wish/RequestCurrentWorkPermitData', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
            ...
            if (res.ok) {
                const result = await res.json();
                return [result.currentWorkPermitList, result.success, result.message];
            }
        }   
        catch (e) {
            console.log(e);
            return [null, false, "requestCurrentWorkPermitData 실패"];
        }
    }

    static async requestCCTVListFromSensor(sensorNo) {
        try {
            const jsonData = SdmsJsonManager.makeRequestCCTVListFromSensor(sensorNo);

            const res = await fetch('SDMS/SDMS/RequestCCTVList', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
            ...
            if (res.ok) {
                ...
                if (result.success) {
                    return [result.cctvs, result.equipZoneNo, result.sensorZoneNo, result.message];
                }
                else {
        ...
        return [null, null, null, "requestCCTVListFromSensor 실패"];
    }

    static async requestEquipZoneCCTVList(equipZoneNo) {
        try {
            const jsonData = SdmsJsonManager.makeRequestEquipZoneCCTVList(equipZoneNo);

            const res = await fetch('api/SDMS/RequestEquipZoneCCTVList', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
            ...
            if (res.ok) {
                ...
                if (result.success) {
                    return [result.cctvs, result.message];
                }
                else {
                    return [null, result.message];
                }
        ...
        return [null, "requestEquipZoneCCTVList 실패"];
    }

    static async requestSensorZoneCCTVList(sensorZoneNo) {
        try {
            const jsonData = SdmsJsonManager.makeRequestSensorZoneCCTVList(sensorZoneNo);

            const res = await fetch('api/SDMS/RequestSensorZoneCCTVList', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return [null, "requestSensorZoneCCTVList 실패"];
    }
    
    static async requestDustMeasurementInfo() {
        try {
            const res = await fetch('SDMS/SDMS/RequestDustMeasurementInfo', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
    ...
    static async requestDustMeasurementHistory(sensorNo, sensorSubType) {
        try {
            const jsonData = SdmsJsonManager.makeRequestDustMeasurementHistory(sensorNo, sensorSubType);

            const res = await fetch('SDMS/SDMS/RequestDustMeasurementHistory', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
    ...
    static async requestWaterGatherInfo(sensorNo) {
        try {
            const jsonData = SdmsJsonManager.makeRequestWaterGatherInfo(sensorNo);

            const res = await fetch('SDMS/SDMS/RequestWaterGatherInfo', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
    ...
    static async requestDustForecastInfo(sensorNo, sensorSubType) {
        try {
            const jsonData = SdmsJsonManager.makeRequestDustForecastInfo(sensorNo, sensorSubType);

            const res = await fetch('SDMS/SDMS/RequestDustForecastInfo', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
    ...
    static async requestWaterGatherForecastInfo(sensorNo) {
        try {
            const jsonData = SdmsJsonManager.makeRequestWaterGatherForecastInfo(sensorNo);

            const res = await fetch('SDMS/SDMS/RequestWaterGatherForecastInfo', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
    ...
    // equipZoneCCTVs : [
    //      {
    //          equipZoneNo(int): [cctv1, cctv2, cctv3, cctv4] // 최대 4개까지...
    //      }
    // ]
    static async saveEquipZoneCCTVList(equipZoneCCTVs) {
        try {
            const jsonData = SdmsJsonManager.makeSaveEquipZoneCCTVList(equipZoneCCTVs);

            const res = await fetch('api/SDMS/SaveEquipZoneCCTVList', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return [false, "saveEquipZoneCCTVList 실패"];
    }

    // sensorZoneCCTVs : [
    //      {
    //          sensorZoneNo(int): [cctv1, cctv2, cctv3, cctv4] // 최대 4개까지...
    //      }
    // ]
    static async saveSensorZoneCCTVList(sensorZoneCCTVs) {
        try {
            const jsonData = SdmsJsonManager.makeSaveSensorZoneCCTVList(sensorZoneCCTVs);

            const res = await fetch('api/SDMS/SaveSensorZoneCCTVList', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return [false, "saveSensorZoneCCTVList 실패"];
    }

    static async UpdateEquipZoneList(updateDatas) {
        try {
            const jsonData = SdmsJsonManager.makeUpdateEquipZoneList(updateDatas);

            const res = await fetch('api/SDMS/UpdateEquipZoneList', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return [false, "UpdateEquipZoneList 실패"];
    }

    static async requestDoorStatus(zoneNo = null) {
        try {
            const jsonData = SdmsJsonManager.makeRequestDoorStatus(zoneNo);

            const res = await fetch('SDMS/SDMS/RequestDoorStatus', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
            ...
            if (res.ok) {
                ...
                if (result.success) {
                    return [result.doorDatas, result.message];
                }
                else {
                    return [null, result.message];
                }
        ...
        return [null, "requestDoorStatus 실패"];
    }

    static async requestComingHistory() {
        try {
            const res = await fetch('SDMS/ComingPerson/RequestComingHistory', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return [null, "requestComingHistory 실패"];
    }

    static async requestAreaComingHistory() {
        try {
            const res = await fetch('SDMS/ComingPerson/RequestAreaComingHistory', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return [null, "requestAreaComingHistory 실패"];
    }

    static async requestLastComingPerson() {
        try {
            const res = await fetch('SDMS/ComingPerson/RequesttLastComingPerson', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return [null, "requestLastComingPerson 실패"];
    }

    // 출입차량 현황정보 조회
    static async requestCurrentParkingInfo() {
        try {
            const res = await fetch('SDMS/SDMS/RequestCurrentParkingInfo', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return [null, "requestCurrentParkingInfo 실패"];
    }

    // [미사용] 차량 상세 이미지 조회 (성공 시 jpeg, 실패 시 json 반환 → ObjectURL 또는 null)
    // 이미지가 파일(/resource/image/parking/{parkingHisNo}.jpg) 방식으로 전환되어 parkingInfo.jsx에서 더 이상 호출하지 않음
    static async requestParkingImage(parkingHisNo) {
        try {
            const res = await fetch('SDMS/SDMS/RequestComingHistory', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ ParkingHisNo: parkingHisNo })
            });

            if (res.ok) {
                const contentType = res.headers.get('Content-Type') || '';

                // 이미지가 존재할 때만 jpeg 바이트를 반환한다. (없으면 json)
    ...
    // 수동 출차 처리
    static async requestParkingManual(parkingHisNo) {
        try {
            const res = await fetch('SDMS/SDMS/RequestParkingManual', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return [null, "requestParkingManual 실패"];
    }

    static async requestRouteHistory(cardNo) {
        try {
            const jsonData = SdmsJsonManager.makeRequestRouteHistory(cardNo);

            const res = await fetch('SDMS/ComingPerson/RequestRouteHistory', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
    ...
    static async requestElevators() {
        try {
            const res = await fetch('SDMS/SDMS/RequestElevators', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
            ...
            if (res.ok) {
                ...
                if (result.success) {
                    return [result.elevators, result.message];
                }
                else {
                    return [null, result.message];
                }
        ...
        return [null, "requestElevators 실패"];
    }

    static async requestTotalDoorStatus() {
        try {
            const res = await fetch('SDMS/SDMS/RequestTotalDoorStatus', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
            ...
            if (res.ok) {
                ...
                if (result.success) {
                    return [result.totalCloseDoorCount, result.zones, result.message];
                }
                else {
                    return [null, null, result.message];
                }
        ...
        return [null, "requestTotalDoorStatus 실패"];
    }

    static async requestTpsZoneList() {
        try {
            ...
            if (res.ok) {
                ...
                if (result.success) {
                    return [result.tpsList, result.message];
                }
                else {
                    return [null, result.message];
                }
        ...
        return [null, "requestTpsZoneList 실패"];
    }

    // TPS실 상위 장비 리스트 가져오기
    static async requestTpsParentEquipments(tpsNo) {
        try {
            const res = await fetch('Equipment/RequestTpsParentEquipments', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ tpsNo: tpsNo })
            });
            ...
            if (res.ok) {
                ...
                if (result.success) {
                    return [result.equipmentList, result.tpsName, ""];
                }
                else {
                    return [null, "", result.message];
                }
        ...
        return [null, "requestTpsParentEquipments 실패"];
    }

    // TPS실 상위-하위 장비 조회
    static async requestTpsEquipments(parentNo) {
        try {
            const res = await fetch('Equipment/RequestTpsEquipments', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ parentNo: parentNo })
            });
        ...
        return [null, "requestTpsEquipments 실패"];
    }
}