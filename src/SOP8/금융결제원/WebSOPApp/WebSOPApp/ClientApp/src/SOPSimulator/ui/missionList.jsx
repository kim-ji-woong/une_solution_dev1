...
function MissionList(props) {
    ...
    useEffect(() => {
        ...
        if (props.commonSettings?.UseAutoMoveSOPScreen?.value?.toLowerCase() === 'true') {
    ...
    const setMissionUI = () => {
        ...
        for (let i = 0; i < sectionsLength; i++) {            
            if (SectionData.isProcessType(props.sections[i].component.compn_code)) {
                list.push(
                    <Process
                        key={i}
            ...
            else if (SectionData.isDecisionType(props.sections[i].component.compn_code)) {
                // 판단
                list.push(
                    <Decision
                        key={i}
            ...
            else if (SectionData.isEndpointType(props.sections[i].component.compn_code)) {
                // 시작, 종료
                list.push(
                    <EndPoint
                        key={i}
            ...
            else if (SectionData.isTransmissionType(props.sections[i].component.compn_code)) {
                // 상황 전파
                list.push(
                    <Internal
                        key={i}
