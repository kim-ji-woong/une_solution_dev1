			...
			for (let i = 0; i < dataSource.length; i++) {
				...
				ui.push(
					<tr key={'dataSource_' + (data.rowNo)} className={data.checked ? ' colorOn' : ''}>
						...
						<td>{data.detectCount ?? '-'}</td>
						<td>{data.malfunctionCount ?? '-'}</td>
						<td>{data.sensorClearCount ?? '-'}</td>
						<td>{data.userResetCount ?? '-'}</td>
						<td>{data.malfunctionRatio ?? '-'}</td>
					</tr>
				);
			...
			<div className='contents'>
				...
				{displayContent && displayContent.length > 0 ?
					...
					<div className='hscTb emptyContent'>
						<EmptyContent
							title="선택한 조건에 해당하는 이벤트 탐지 분석이 존재하지 않습니다."
