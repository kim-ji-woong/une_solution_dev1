...
import EquipmentManager from '../EquipmentManager/ui/equipmentManager';
...
function Menu(props) {
    ...
    const [equipmentManagerEvent, setEquipmentManagerEvent] = useState({});
    ...
    const getTargetEvent = () => {
        ...
        if (path.length > 0) {
            ...
            else if (path === ProjectResource.path.equipmentManager) 
                return [equipmentManagerEvent, target];
    ...
    const getRoutePath = () => {
        return (<React.Fragment>
            <Route path={ProjectResource.path.sdms} render={() => <SDMS menuEvent={sdmsEvent} />} />
            <Route path={ProjectResource.path.history} render={(routeProps) => <History {...routeProps} menuEvent={historyEvent} />} />
            ...
            <Route path={ProjectResource.path.equipmentManager} render={() => <EquipmentManager menuEvent={equipmentManagerEvent} />} />
