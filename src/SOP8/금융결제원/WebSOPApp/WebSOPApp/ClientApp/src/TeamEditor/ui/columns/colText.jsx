...
function ColText(props) {
    ...
    const onBlurCheck = (e) => {
        ...
        if (columnName === '휴대전화번호') {
            // 숫자만 추출
            const onlyNumber = target.value.replace(/[^0-9]/g, '');
        
            // 중복 검사
            if (onlyNumber !== '') {
                const chk = props.checkPhoneNumber(props.member.rgl_memb_sn, onlyNumber, target);
            ...
            props.member.telno = onlyNumber;
        }
        else if (columnName === '근무처 전화번호') {
            // 숫자만 추출
            const onlyNumber = target.value.replace(/[^0-9]/g, '');
            props.member.offm_telno = onlyNumber;
        }
