...
class SensorSimulator extends Component {
    ...
    async checkAlarmList() {
        const [alarmList, message] = await SDMSController.requestTodayAlarmData();
    ...
    setSensorList(sensors, _3dOptions, sensorList, facilityType) {
        for (const sensor of sensors) {
            ...
            for (const siteID in _3dOptions) {
                ...
                const outdoorData = _3dOption.outdoorZones[sensor.sensor.zone_sn];
                if (outdoorData) {
                    this.setOutdoorZoneData(outdoorData, sensor, buildingGroups);
                    break;
                }
            }
    ...
    setOutdoorZoneData(data, sensor, sensors) {
        const zoneName = data.name;

        if (zoneName) {
            let zoneSensors = sensors[zoneName];

            if (!zoneSensors) {
                zoneSensors = {
                    isOutdoor: true
                };
                sensors[zoneName] = zoneSensors;
            }
    ...
    getSensorElements() {
        ...
        for (const buildingGroupName of this.getSortedKeys(sensors)) {
            ...
            const isOutdoor = buildingGroupData.isOutdoor;
            ...
            if (isOutdoor) {
                ...
                for (const sensorName of this.getSortedKeys(buildingGroupData)) {
                    if (sensorName === "isOutdoor") continue;

                    const _sensor = buildingGroupData[sensorName];
                    const sID = "tabS_" + sIndex++;
                    const sensorClassName =
                        _sensor === this.state.selectedSensor
                ...
                buildingGroupElements.push(
                    <ul key={'outdoorSensors_' + bgID} className={sensor.thirdTabOutdoor}>
                        {sensorElements}
                    </ul>
                );

                buildingGroupLiElements.push(
                    <li key={'buildingGroupLi_' + bgID}>{buildingGroupElements}</li>
                );

                // outdoor 이면 밑의 depth (building -> zone) 탐색을 건너뛰고 다음 그룹으로 넘어감
                continue;
            }
    ...
    onClickSendAlarm = async (isAlarm, allClear) => {
        if (isAlarm) {
            ...
            if (!selectedSensor) {
                ...
                if (selectedSensor.sensorZoneData?.sensorZone) {
