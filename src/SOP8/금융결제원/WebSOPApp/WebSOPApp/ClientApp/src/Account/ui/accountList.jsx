...
function AccountList(props) {
    ...
    const selectedUser = props.accountUsers.find(
        user => user.userNo === selectedUserNo
    );
    ...
    useEffect(() => {
        ...
        if (props.accountUsers && props.accountUsers.length > 0) {
            ...
            for (const user of props.accountUsers) {
                ...
                ui.push(
                    <li key={user.userNo} onClick={(e) => updateUser(e, user)}>
                        <div>{index}</div>
    ...
    const handlePopup = (isShow) => {
        if (!isShow) {
            ...
            Array.from(element).forEach(item =>
                item.classList.remove('selectUser')
            );
        }
    ...
    return (
        ...
        {showUpdateUserPopup && selectedUser &&
            <AccountUpdateUser
