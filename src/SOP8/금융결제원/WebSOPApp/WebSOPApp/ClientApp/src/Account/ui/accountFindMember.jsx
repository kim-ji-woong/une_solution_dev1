...
function AccountFindMember(props) {
    ...
    useEffect(() => {
        if (props.type === 'spread') {
            props.setRegularMembers(''); // 초기 전체 조회
        }
    }, []);

    useEffect(() => {
        const list = props.type === 'spread'
            ? [...(props.regularMembers || [])].sort((a, b) => {
                const aChecked = selectedSet.has(a.rgl_memb_sn);
                const bChecked = selectedSet.has(b.rgl_memb_sn);

                if (aChecked === bChecked) return 0; // 둘 다 같으면 순서 유지
                return aChecked ? -1 : 1; // 체크된 애를 위로
            })
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            if (props.mode === 'edit') {
                                                ...
                                                // 신규등록
                                                props.onCheckedRow?.(e.target.checked, member.rgl_memb_sn);
                                            }
                                        }}
                                    />
                                )
