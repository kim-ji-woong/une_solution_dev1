...
function FindPwdSection(props) {
    ...
    useEffect(() => {
        const msg = document.querySelector('.errorMsg p');

        if (msg) {
            msg.style.color = "";
        }

        if (props.errorMsg === AccountResource.ID.textFindPwdSuccess) {
            // 성공 메시지 색상 변경
            if (msg) msg.style.color = "#3C69FC";
        ...
        if (props.errorMsg === AccountResource.ID.textFindPwdError) {
            refName.current.classList.add('error');

            mode === AccountResource.findMode.email
                ? refEmail.current?.classList.add('error')
                : refPhone.current?.classList.add('error');
        }
    }, [props.errorMsg]);
    ...
    const content = (
        <FindPwdSectionComponent>
            <div className='headerWrap'>
                <Button className="cancleBtn" variant="unfill" size="md" leftIcon={<Icon.Arrow size={"xxxs"} direction={"left"} />} onClick={() => onChangeSection()}>
                    취소하고 돌아가기
                </Button>
            </div>
