...
function AccountUpdateUser(props) {
    ...
    const handleEditMode = () => {
        ...
        setGradeNo(initialGradeNoRef.current);
        ...
        <AccountUpdateUserComponent $isEditMode={isEditMode}>
            <header>
                ...
                <div>
                    {!isEditMode && (
                        <>
                            {/* 상위 권한 및 동일 권한은 편집/삭제 불가하므로 버튼 숨김 */}
                            {props.loginUserInfo?.grad_sn < props.selectedUser?.gradeNo && (
                                <>
                                    <IconButton
                                        variant="unfill"
                                        size="xxs"
                        ...
                        <div>
                            {
                                isEditMode ? 
                                    <select value={gradeNo} onChange={onChangeGrade}>
                                        {/* 로그인 사용자의 권한과 같거나 낮은 권한 등급만 표출 */}
                                        {props.grades?.map(grade =>
                                            props.loginUserInfo.grad_sn <= grade.grad_sn ? (
