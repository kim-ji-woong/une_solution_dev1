...
namespace SoulbrainIoT
{
    ...
    public class WebServiceManager
    {
        ...
        private List<DataSensor> GetDeviceSensorList(DataDevice device, JArray jArrSensor, bool bChkAlarm = false)
        {
            ...
            if (alarms == null)
            {
                m_parent.Logger.Write("GetAlarmList : " + strErrorMessage);
            }
        ...
        private bool CheckAlarmData(DataDevice device, DataSensor sensor, List<AlarmData> alarms)
        {
            ...
            if (alarm != null && (sensor.SensorStatus == SoulbrainID.STATUS_NORMAL || sensor.SensorStatus == SoulbrainID.STATUS_OFFLINE))
            {
                ...
                if (alarmData == null)
                {
                    m_parent.Logger.Write($"CheckAlarmData GetAlarmData Error ({strErrorMessage})");
                    return false;
                }
                else
                {
                    if (m_parent.SendSensorData(alarmData.SensorZoneID, alarmData.SensorType, false, out strErrorMessage) == false)
                    {
                        m_parent.Logger.Write($"SendSensorData Error : {strErrorMessage} (SensorType: {alarmData.SensorType.ToString()}, SensorZoneID: {alarmData.SensorZoneID.ToString()})");
                        return false;
                    }
                    else
                    {   // 알람 로그 작성
            ...
            if (sensor.SensorStatus == SoulbrainID.STATUS_CAUTION || sensor.SensorStatus == SoulbrainID.STATUS_WARNING)
            {
                ...
                else
                {
                    ...
                    if (m_parent.SendSensorData(alarmData.SensorZoneID, alarmData.SensorType, true, out strErrorMessage, nAlarmLevel) == false)
                    {
                        m_parent.Logger.Write($"SendSensorData Error : {strErrorMessage} (SensorType: {alarmData.SensorType.ToString()}, SensorZoneID: {alarmData.SensorZoneID.ToString()})");
                        return false;
                    }
        ...
        private void WriteAlarmLog(DataDevice device, DataSensor sensor, bool bIsRun)
        {
            ...
            {   // 처음 작성
                ...
                // 로그 작성
                m_parent.Logger.Write(strAlarmLog);
            }
            else
            {
        ...
        public bool SendAlarmSensorData(List<AlarmSensorData> alarmSensors, out string strErrorMessage)
        {
            ...
            foreach (AlarmSensorData alarmSensorData in alarmSensors)
            {
                ...
                if (alarmData != null)
                {
                    ...
                    if (m_parent.SendSensorData(alarmData.SensorZoneID, alarmData.SensorType, bIsAlarm, out strErrorMessage, nAlarmLevel) == false)
                    {
                        m_parent.Logger.Write($"SendSensorData Error {strErrorMessage}");
                        continue;
                    }
                ...
                {
                    m_parent.Logger.Write($"SendAlarmSensorData GetAlarmData Error {strErrorMessage}");
                }
            }
        ...
        public bool RequestSensorData(List<AlarmSensorData> alarmSensors, out string strErrorMessage)
        {
            ...
            foreach (AlarmSensorData alarmSensor in alarmSensors)
            {
                ...
                {
                    m_parent.Logger.Write($"RequestSensorData SendQuery Error (Device ID: {alarmSensor.DeviceId}) : {strErrorMessage}");
                }
            }
