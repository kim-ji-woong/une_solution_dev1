...
namespace Johnson
{
    /// <summary>
      /// 화재-동방 통신
    /// </summary>
    class JohnsonProvider : ClientServiceProvider
    {
        ...
        public JohnsonProvider(JohnsonManager mgr, int muxType)
        {
            m_parentManager = mgr;
            m_muxType = (MuxTypes)muxType;
        ...
        {
            m_parentManager.Logger.Write("close Connection");
        }
        ...
        public void ProcessData(byte[] bytes)
        {
            ...
            {
                m_parentManager.Logger.Write("ProcessData(byte[]) : " + ex.Message);
            }
        }
        ...
        private void ProcessData(byte[] bytes, int nBeginIndex, int nEndIndex)
        {
            try
            {
                ...
                m_parentManager.Logger.Write(strLog);
            ...
            {
                m_parentManager.Logger.Write("ClientProvider.cs > void ProcessData(byte[], int, int) :" + ex.Message);
            }
        }

        public void ProcessFire(int nReceiverID, int nRelayTeam, int nLoopID, int nRelayID, int nTagID, bool isAlarm)
        {
            int nTagNo = GetJohnsonSensorTagNo(nReceiverID, nRelayTeam, nLoopID, nRelayID, nTagID);

            string strErrorMessage;
            SensorZone sensorZone = m_parentManager.FindSensorZone(nTagNo.ToString(), out strErrorMessage);

            if (sensorZone != null)
            {
                if (m_parentManager.SendSensorData(sensorZone, sensorZone.sensor_ty_code, isAlarm, out strErrorMessage) == false)
                {
                    m_parentManager.Logger.Write("ProcessFire : " + strErrorMessage);
                }
                else
                {
                    m_parentManager.Logger.Write("SendSensorData Error  : " + strErrorMessage);
                }
            }
            else
            {
                m_parentManager.Logger.Write("FindSensorZone Error : " + strErrorMessage + " (SensorZone :" + nTagNo.ToString() + ")");
            }
        }
        ...
        private void ProcessAllClear()
        {
            ...
            if (m_parentManager.SendAllClear(siteID, out string strErrorMessage) == false)
            {
                m_parentManager.Logger.Write("ProcessAllClear : " + strErrorMessage);
            }
        }
        ...
        private void WriteBinaryLog(byte[] bytes, int nIndex, int len)
        {
            ...
            strLog = string.Format("Recv from Server\r\nBytes Length : {0}\r\n{1}", len, strLog);
            m_parentManager.Logger.Write(strLog);
        }
