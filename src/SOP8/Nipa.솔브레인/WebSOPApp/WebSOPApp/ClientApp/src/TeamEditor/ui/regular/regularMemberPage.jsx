							...
							<li className='head'>
								...
								<div>
									...
									<button onClick={() => onClickSort('teamName')}>
										<Icon.SortIcon direction={getSortDirection('teamName')} />
									</button>
								</div>
								<div>
									<span>이름</span>
									<button onClick={() => onClickSort('memb_name')}>
										<Icon.SortIcon direction={getSortDirection('memb_name')} />
									</button>
								</div>
								<div>
									<span>직위</span>
									<button onClick={() => onClickSort('ofcps_name')}>
										<Icon.SortIcon direction={getSortDirection('ofcps_name')} />
									</button>
								</div>
								<div>
									<span>직급</span>
									<button onClick={() => onClickSort('clsf_name')}>
										<Icon.SortIcon direction={getSortDirection('clsf_name')} />
									</button>
								</div>
								<div>
									<span>휴대전화</span>
									<button onClick={() => onClickSort('telno')}>
										<Icon.SortIcon direction={getSortDirection('telno')} />
									</button>
								</div>
								<div>
									<span>사무실전화</span>
									<button onClick={() => onClickSort('offm_telno')}>
										<Icon.SortIcon direction={getSortDirection('offm_telno')} />
									</button>
								</div>
								<div>
									<span>이메일</span>
									<button onClick={() => onClickSort('email')}>
										<Icon.SortIcon direction={getSortDirection('email')} />
									</button>
								</div>
								<div>
									<span>사번</span>
									<button onClick={() => onClickSort('unq_key')}>
										<Icon.SortIcon direction={getSortDirection('unq_key')} />
									</button>
								</div>
							</li>
							<li className='body'>
								{displayContent.length > 0 ?
									...
									<ul className='noData'>
										<li>
											...
											<p>선택한 조직에 속한 사용자가 없습니다</p>
											<p>추가하기 버튼을 이용해 사용자를 추가하세요</p>
										</li>
									</ul>
								}	
							</li>
