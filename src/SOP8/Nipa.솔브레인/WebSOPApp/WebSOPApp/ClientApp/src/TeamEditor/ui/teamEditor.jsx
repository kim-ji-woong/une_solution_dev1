...
function TeamEditor(props) {
	...
	const [roles, setRoles] = useState(null);					// 주/부 정보
	...
	const init = async () => {
		...
		const { arrJobLevels, arrJobPositions } = await displayJob();

		setSelectedSiteNo(userInfo.site_sn ?? ProjectResource.site_sn);

		await initTeamData(arrJobLevels, arrJobPositions);
	};
	...
	const displayJob = async () => {
		...
		return { arrJobLevels, arrJobPositions };
	}

	const initTeamData = async (jobLevels, jobPositions) => {
		await loadRegularTeam(jobLevels, jobPositions);
	};

	const displayRegular = async () => {
		await loadRegularTeam(jobLevels, jobPositions, { resetOnEmpty: true });
	};

	const loadRegularTeam = async (jobLevels, jobPositions, { resetOnEmpty = false } = {}) => {
		...
		const selectedTeam = teamDatas[0];
		const displayMembers = getViewRegularMember(
			selectedTeam,
			members
		);
