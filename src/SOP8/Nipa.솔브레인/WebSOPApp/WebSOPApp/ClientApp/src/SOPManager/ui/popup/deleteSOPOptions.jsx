...
const DeleteSOPOptions = forwardRef((props, ref) => {
	...
	useEffect(() => {
		if (!props.isAction) setIsClicked(false); // 삭제 끝나면 다시 버튼 활성화
	}, [props.isAction]);
	...
	const onClickDelete = (event) => {
		...
		if (versionIDs && versionIDs.length > 0) {
			if (checkCurrentVersion(versionIDs)) {
				props.content(SopManagerResource.menu.delete, [versionIDs, null, isNormal]);
			}
        }
		else {
			props.showConfirmDialog(ProjectResource.dialogTypes.WARNING, ['삭제할 SOP를 선택해주세요.'], null, null);
	...
	const onConfirmDelete = (index) => {
		if (index === 1) {
			props.content(SopManagerResource.menu.delete, [deleteVersionIDs, null, isNormal]);
		}
							...
							<div className={'sppCont'}>
								...
								<div className={'sppRhtDelete'}>
									...
									<div className={'spprBot'}>
										<a className={'cancel'} onClick={onClickClose}>취소</a>
										<a
