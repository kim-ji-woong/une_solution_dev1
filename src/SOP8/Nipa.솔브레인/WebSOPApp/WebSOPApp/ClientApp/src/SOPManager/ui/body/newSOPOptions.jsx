...
import etc_icon from '../../images/etc.png';
import explosion_icon from '../../images/explosion.png';
import natureDisaster_icon from '../../images/natureDisaster.png';
import pollution_icon from '../../images/pollution.png';
import security_icon from '../../images/security.png';
import terror_icon from '../../images/terror.png';
import lifesaving_icon from '../../images/lifesaving.png';
import earthquake_icon from '../../images/earthquake.png';
import strongwind_icon from '../../images/strongwind.png';
import blackout_icon from '../../images/blackout.png';
...
function NewSOPOptions(props) {
	...
	const onSelectNewDisaster = () => {
		const newDisaster = {
			disaster:
			{
				sclas_sn: -1,
				sclas_name: "",
				mclas_sn: selectedSubDisasterCategory?.subDisasterCategory?.mclas_name,
				ver_sn: -1,
				userLevelNos: null,
				descp: null
			},
	...
	const getSOPData = () => {
		if (selectedSubDisasterCategory === newSubDisasterCategory) {
			...
			selectedSubDisasterCategory.subDisasterCategory.mclas_name = sdcName;
		}
		...
		if (disasterData === newDisaster) {
			...
			disasterData.disaster.sclas_name = disasterName;
			disasterData = {
		...
		disaster.disaster.nor_yn = isNormal;
		disaster.disaster.sclas_sn = disaster.disaster.sclas_sn;
		disaster.disaster.ver_sn = disaster.disaster.ver_sn;

		delete disaster.disaster.sclas_sn;
		delete disaster.disaster.ver_sn;
	...
	const addActionStepDatas = async (sopData) => {
		const [actionStepDatas, message] = await SopController.requestDefaultActionStepDatas(props.selectedSiteNo);
		...
		if (sopData) {
			...
			for (let i = 0; i < actionStepCount; i++) {
				...
				if (actionStepData.stepMemberDatas.length === 0) {
					const stepMemberData = await SopController.requestDefaultStepMemberData(actionStepData/*, props.selectedSiteNo*/);
					...
					for (const stepMemberData of actionStepData.stepMemberDatas) {
						stepMemberData.rawSections = [];
						stepMemberData.sections = [];
						stepMemberData.arrows = [];
                    }
                }
			}
		}
	...
	const getDisasterCategoryImage = (disasterCategoryData) => {
		const dcType = SopManagerResource.getDisasterCategoryType(disasterCategoryData.disasterCategory.lclas_name);
	...
	const onChangeDisasterText = () => {
		...
		// input에 focus만 되어도 radio button check 되도록 수정 - 한봄희
		...
		<NewSOPOptionsComponent>
			<div className={'speTop'}>
				...
				<div className={'labelInputRadio'}>
					<input type="radio" name="speTop" /* id={NewSOPOptions.cssStyles.speTop01} */ checked={isNormal} onChange={() => onSelectSopMode(null, true)} /><label onClick={(event) => onSelectSopMode(event, true)}>평일/주간</label>
				</div>
				<div className={'labelInputRadio'}>
					<input type="radio" name="speTop" /* id={NewSOPOptions.cssStyles.speTop02} */ checked={!isNormal} onChange={() => onSelectSopMode(null, false)} /><label onClick={(event) => onSelectSopMode(event, false)}>휴일/야간</label>
				</div>
			</div>
									...
									<div className={'speGry'}>
										{
											disasterCategories.map(disasterCategoryData => (
												<li key={"dc_" + disasterCategoryData.disasterCategory.lclas_name}>
													<label htmlFor={'speGry01_' + disasterCategoryData.disasterCategory.lclas_name}>
														<span>
															<input type="radio" name="speGry" id={'speGry01_' + disasterCategoryData.disasterCategory.lclas_name} onChange={() => onSelectDisasterCategory(disasterCategoryData)} />
														</span>
														<span>{disasterCategoryData.disasterCategory.lclas_name}</span>
													</label>
												</li>
												))
										}
									...
									<div className={'speLst'}>
										{
											subDisasterCategories.map(sdc => (
												<li key={"sdc_" + sdc.subDisasterCategory.mclas_name}>
													...
													<label onClick={(event) => onSelectSubDisasterCategory(event, sdc)}>{sdc.subDisasterCategory.mclas_name}</label>
													</span>
												</li>
											))
										}
									...
									<div className={'speIpt'}>
										{
											disasterDatas.map(disasterData => (
												<li key={"disaster_" + disasterData.disasterDatas[0].disaster.sclas_sn}>
													<span className={'labelInputRadio'}>
										...
										<span className={'labelInputTextBlack'}>
											<input ref={refNewDisaster} type="text" disabled={selectedSubDisasterCategory === null} placeholder={SopManagerResource.ID.placeHolders.newSOP} onChange={() => onChangeDisasterText()} onFocus={() => onChangeDisasterText()} onKeyDown={(e) => activeEnter(e)} />
										</span>
										</li>
									</div>
								</div>
			...
			<div className={'speBot'}>
				...
				>
					{SopManagerResource.ID.common.cancel}
				</Button>
				<Button
					variant='fill'
					size='md'
				...
				>
					{SopManagerResource.ID.common.make}
				</Button>
			</div>
		</NewSOPOptionsComponent>
	);
