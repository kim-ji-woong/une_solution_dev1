...
function SaveSOPOptions(props) {
	...
	useEffect(() => {
		if (!props.isAction) setIsClicked(false); // 저장 끝나면 다시 버튼 활성화
	}, [props.isAction]);
	...
	const handleSave = () => {
		if (isClicked || props.isAction) return; // 이미 눌렸으면 차단
		setIsClicked(true);
		onClickApply(true);
	};
	...
	const onClickApply = (ok) => {
		if (ok) {
			...
			if (selectedVersion) {
				sopData.version.ver_sn = selectedVersion.version.ver_sn;
				sopData.version.name = selectedVersion.version.name;
				props.content(SopManagerResource.menu.save, sopData);
			} else {
						...
						<div className={'spPop'}>
							...
							<div className={'spprCont2'}>
											...
											<table className={'scTb'}>
												...
												<tbody>
													...
													{versions.map((item, idx) => {
														...
														return (
															<tr key={v.ver_sn} onClick={(event) => tbRdo(event, item)}>
																<td>{v.name}</td>
																<td>{item.owner}</td>
																<td>{v.creat_de.replace('T', ' ')}</td>
																<td>{v.last_acces_de.replace('T', ' ')}</td>
																<td>{v.descp}</td>
															</tr>
														);
													})}
								...
								<div className={'spprBot'}>
									...
									<a
										className={props.isAction || isClicked ? 'submit disabled' : 'submit'}
										onClick={handleSave}
									>
										저장하기
									</a>
