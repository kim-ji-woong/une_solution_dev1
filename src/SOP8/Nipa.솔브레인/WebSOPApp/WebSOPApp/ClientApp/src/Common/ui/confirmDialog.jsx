...
export default function ConfirmDialog({ type, messages, buttons, onClickButton, onCloseConfirmDialog, isManualAlarm = false, hideMalfunctionCheck = false }) {
    ...
    */
    ...
    const handleClick = (e, index) => {
        ...
        if (onClickButton) {
            onClickButton(index, isMalfunction);
        }

        onCloseConfirmDialog(); // 항상 닫기
    };
    ...
    const getButtons = () => {
        if (!Array.isArray(buttons) || buttons.length === 0) {
            return (
                <footer>
                    <button
                        className="confirmBtn confirm_0"
                        onClick={(e) => {
                            if (e.currentTarget.disabled) return;
                            e.currentTarget.disabled = true;
        ...
        return (
            <footer>
                {type === ProjectResource.dialogTypes.MALFUNCTION && !hideMalfunctionCheck && (
                    <div key="checkbox" className="malfunctionWrap">
                        ...
                        />
                        <label htmlFor="malfunctionCheck">
                            {isManualAlarm ? '잘못된 신고로 처리하기' : '오작동 처리하기'}
                        </label>
                    </div>
                )}
