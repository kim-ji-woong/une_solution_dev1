...
import { getProjectTypeState } from './resource/projectType';

import { TitleBarComponent } from './styled/titleBarStyled';
import soulbrain_logo from '../Common/images/soulbrain_logo_white.png';
...
function TitleBar(props) {
    ...
    const getCurrentMenuName = () => {
        ...
        if (path === ProjectResource.path.sdms) {
            ...
            return <p>{ProjectResource.ID.title.dashboard}</p>
        }
    ...
    const getMenu = () => {
        const menuItems = projectTypeState.isFacilityProject
            ? [
                { path: ProjectResource.path.sdms, title: ProjectResource.ID.title.sdms },
                { path: ProjectResource.path.history, title: ProjectResource.ID.title.history },
            ]
            : [
                { path: ProjectResource.path.sdms, title: ProjectResource.ID.title.sdms },
                { path: ProjectResource.path.dashboard, title: ProjectResource.ID.title.dashboard },
                { path: ProjectResource.path.sopSimulator, title: ProjectResource.ID.title.sopSimulator },
                { path: ProjectResource.path.sopManager, title: ProjectResource.ID.title.sopManager },
                { path: ProjectResource.path.history, title: ProjectResource.ID.title.history },
                { path: ProjectResource.path.teamEditor, title: ProjectResource.ID.title.teamEditor },
            ];

        const handleMenuClick = (path) => {
            if (path === ProjectResource.path.history) {
        ...
        return (
            <div id='navMenu' className={showMenuPopup ? 'on' : 'off'}>
                <ul>
                    {menuItems.map(item => (
                        <li key={item.path} onClick={() => handleMenuClick(item.path)}>
                            {item.title}
                        </li>
                    ))}
                    {!projectTypeState.isFacilityProject &&
                        <li>
                            {ProjectResource.ID.title.report}
                        </li>
                    }
                </ul>
            </div>
    ...
    const getAccountManagerBtn = () => {
        ...
        if (projectTypeState.isFacilityProject || !userInfo || userInfo.grad_sn === AccountResource.accountLevelNo.user) {
            return <></>
        }
    ...
    const isDisabledPath =
        window?.location?.pathname === ProjectResource.path.sopSimulator ||
        window?.location?.pathname === ProjectResource.path.sopManager;
            ...
            <TitleBarComponent>
                <div className='logoWrap'>
                    <img 
                        ...
                        width={106}
                        height={20}
                        style={{ cursor: isDisabledPath ? 'default' : 'pointer' }}
                        onClick={!isDisabledPath ? onClickLogo : undefined}
                    />
                    {getCurrentMenuName()}
                </div>
                <div className='contentWrap'>
                    <div className='weatherWrap'>
                        <div className='weatherIcon'>
                            {SdmsResource.getWeatherStateImage(weatherDatas ? weatherDatas?.[0]?.wethr_sttus_code : null, 18)}
                        </div>
                        <Button 
                            variant="unfill"
                            size="xxs"
