...
function UpdateSpread(props) {
    ...
    // 편집 화면에 표시할 리스트
    const displayMembers = isEditMode
    ...
    useEffect(() => {
        ...
        initialMembersRef.current = props.selectedSpread.regularMembers || [];
    ...
    const getSpreadMembers = () => {
        if (!isEditMode && displayMembers.length === 0) {
            return null;
        }
    ...
    return (
        <ModalBackground>
            <UpdateSpreadComponent $isEditMode={isEditMode}>
                <header>
                    <h2>{isEditMode ? '상세정보 편집하기' : '상세정보'}</h2>
                    <div>
                        {
                            !isEditMode &&
                                <>
