...
function BeginOption(props) {
    ...
    const makeTimeString = (year, month, day, hour, min, sec) => {
        const time = year.toString() + "-" + make00(month) + "-" + make00(day) + "T" + make00(hour) + ":" + make00(min) + ":" + make00(sec);
        return time;
    }
    ...
    const onClickBegin = () => {
        ...
        if (!position || position.length === 0) {
            ...
            if (isCurrent) {
                ...
                beginTime = makeTimeString(year, month, day, hour, min, sec);
                ...
                <div>
                    <div className={'sqPop'}>
                        ...
                        <div className={'sqpCont'}>
                            <div className={'sqpUp'}>
                                <input type="text" id="txtPosition" className={'sqpSel'} onKeyUp={beginEnterKey} placeholder='발생위치' />
                            </div>
                            <div className={'sqpDown'}>
                                <ul className={'sqpRdo'}>
                                    <li><input type="radio" name="sqpRdo" id="sqpRdo01" onChange={() => onChangeTimeMode(true)} checked={isCurrent} /><label htmlFor="sqpRdo01">현재시간을 발생일시로 설정</label></li>
                                    <li><input type="radio" name="sqpRdo" id="sqpRdo02" onChange={() => onChangeTimeMode(false)} /><label htmlFor="sqpRdo02">발생일시 직접입력</label></li>
                                </ul>
                                ...
                                <ul className={'sqpBtn'}>
                                    <li><a className={'cancle'} onClick={onClickClose}>취소</a></li>
                                    <li><a className={'submit'} onClick={onClickBegin}>실행하기</a></li>
                                </ul>
                            </div>
                        </div>{/*<!-- sqpCont -->*/}
                    </div>{/*<!-- sqPop -->*/}
