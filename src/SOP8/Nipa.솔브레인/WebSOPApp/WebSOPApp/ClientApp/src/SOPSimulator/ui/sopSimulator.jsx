...
function SopSimulator(props) {
    ...
    const selectedSiteNoRef = useRef(selectedSiteNo);
    ...
    useEffect(() => {
        selectedSiteNoRef.current = selectedSiteNo;
    }, [selectedSiteNo]);
    ...
    const loadSOP = async (storeValue, sopTabKey, actionStep) => {
        ...
        let newSelectedSiteNo = selectedSiteNoRef.current;
        const userInfo = await ProjectResource.initUserInfo();
        if (selectedSiteNoRef.current === null) {
            if (userInfo) {
