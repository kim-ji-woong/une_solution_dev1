...
function AlarmCCTVInfo(props) {
    ...
    const updateTileSize = useCallback(() => {
        const tileBody = document.querySelector(`#${props.popupType} .cctvTile:not(.hidden) .tileBody`);
        if (tileBody) {
            setTileSize({ w: tileBody.clientWidth, h: tileBody.clientHeight });
        }
    }, [props.popupType]);

    useLayoutEffect(() => {
        updateTileSize();
    }, [updateTileSize, expandedIdx]);
    ...
    const buildStreamUrl = (suuid, isExpanded) => {
        ...
        if (url.includes('/stream/player/')) {
            if (tileSize.w === 0 || tileSize.h === 0) return null;
            const joiner = url.includes('?') ? '&' : '?';
            return `${url}${joiner}w=${tileSize.w}px&h=${tileSize.h}px`;
        }
            ...
            <div className={gridClass}>
                {slots.map((item, idx) => {
                    ...
                    if (enabled && suuid) {
                        ...
                        if (!url) {
                            const rawUrl = urlMap.get(String(suuid));
                            const isAwaitingSize = rawUrl?.includes('/stream/player/') && tileSize.w === 0;
                            return (
                                <div
                                    className={tileClass}
                                    key={idx}
                                ...
                                >
                                    ...
                                    <div className="tileBody">{isAwaitingSize ? null : getDisableUI()}</div>
                                </div>
                            );
                        }
    ...
    return (
        <AlarmCCTVInfoComponent id={props.popupType} className="UI_Section alarmCCTVInfo" $resize={true}>
            <PopupDraggable
                id={props.popupType}
                popupMinWidth={340}
                popupMinHeight={320}
                ...
                popupResizeMouseUp={updateTileSize}
            >
