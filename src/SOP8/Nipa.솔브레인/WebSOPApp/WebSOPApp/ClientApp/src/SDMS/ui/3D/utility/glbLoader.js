...
import { CfdLoader } from "./cfdLoader";
...
export class GlbLoader {
    constructor(timelog, scene, props, setState, master3d) {
        ...
        this.facilityManager = master3d.facilityManager;
    }

    loadOutdoorModelFiles(modelFiles, buildingGroupList, gltfOptions, visible) {
        /*if (!TextPoiManager.loadSpriteIconImage()) {
            setTimeout(() => this.loadOutdoorModelFiles(modelFiles, buildingGroupList, gltfOptions, visible), 500);
            return;
        }*/
    ...
    _loadComponentModels() {
        ...
        loader.load(fileName, function (object) {
            ...
            if (fakeWall) {
                ...
                _this._3dMaster.editModeManager.setFakeWallModel(fakeWall, _this._3dMaster.scene);
            }
    ...
    _loadRootModel(contents, nextIndex, files, mode, visible, buildingGroupList, gltfOptions) {
        ...
        loader.load(fileName, function (object) {
            ...
            _this._loadCfdRootModel(_this.getCfdRootModelName(modelNode.name), false, gltfOptions);
    ...
    getCfdRootModelName(rootModelName) {
        const index = rootModelName.lastIndexOf('.');

        if (index < 0)
            return rootModelName + "_cfd";

        return rootModelName.substring(0, index) + "_cfd" + rootModelName.substring(index);
    }

    _loadCfdRootModel(contentsFileName, visible, gltfOptions) {
        const fileName = gltfOptions.modelBaseUrl + "/" + contentsFileName;
        const worldBox = new THREE.Box3();
        ...
        loader.load(fileName, function (object) {
            ...
            modelNode.add(obj);
            modelNode.matrixAutoUpdate = false;
            modelNode.name = contentsFileName;

            modelNode.visible = visible;
            _3dMaster.hideBoundingBoxes(modelNode);
            
            _this.getModelLayer(_3dMaster.Mode_Outdoor_All).add(modelNode);
            modelNode.updateMatrixWorld(true);

            _this._removeBoundingBoxShadow(modelNode);
            _this._3dMaster.setTerrainModel(modelNode);
        });
    }
    ...
    _onCompleteOutdoorModelLoading(modelNode, gltfOptions) {
        ...
        const totalModelCount = this.indoorModelCount + this.outdoorModelCount;

        const rate = this.completeOutdoorModelCount / totalModelCount * 100;
        const rateValue = parseInt(rate);
        this.props.setProgressValue(rateValue);
        ...
        if (this.completeOutdoorModelCount >= this.outdoorModelCount) {
            ...
            const cfdLoader = new CfdLoader(this.scene);
            this._3dMaster.setCfdLoader(cfdLoader);
            //this.cfdLoader.loadCsv("HF_2000s.csv");

            // 파일 읽기
            this.facilityManager.LoadFile();
    ...
    /*setInitialViewport(modelNode) {
        ...
        // 모델 위치를 기준으로 카메라 위치 결정
        const offsetDir = new THREE.Vector3(1, 0.7, 1).normalize();
        const cameraPos = center.clone().add(offsetDir.multiplyScalar(cameraDistance));

        camera.position.copy(cameraPos);
        camera.lookAt(center);
        
        this._3dMaster.controls.target.copy(center);
        this._3dMaster.controls.update();
    }*/
    ...
    _onAfterLoadFile(contentsFile, gltfOptions, mode, postMethod, postMethodParam) {
        if (mode === _3dMaster.Mode_Indoor && this.indoorModelCount > 0) {
            ...
            const totalModelCount = this.indoorModelCount + this.outdoorModelCount;
            const rate = (this.indoorModelCountTemp + this.outdoorModelCount) / totalModelCount * 100;
            ...
            if (this.indoorModelCountTemp === this.indoorModelCount) {
                ...
                const rateValue = parseInt(rate);
                this.props.setProgressValue(rateValue);
                console.log("ProgressValue : " + rateValue);
