...
function Event(props) {
    ...
    const [count, setCount] = useState({all: 0, sensor: 0, report: 0, realTimeAnalysis: 0, predictiveAnalysis: 0});
    ...
    useEffect(() => {
        if (!props.sensorAlarm.selectedAlarm) return;

        const menuType = props.sensorAlarm.selectedAlarm.facilityType === SdmsResource.facilityType.EQUIPMENT_PredictAlarm || props.sensorAlarm.selectedAlarm.facilityType === SdmsResource.facilityType.EQUIPMENT_PeakPower ? "equipment" : "sensor";

        setMenuType(menuType);
    }, [props.sensorAlarm.selectedAlarm])
    ...
    useEffect(() => {
        const renderUI = async () => {
            ...
            let realTimeAlarmCount = 0;
            let predictiveAlarmCount = 0;
            ...
            if (listByMenu?.length > 0) {
                for (const alarm of listByMenu) {
                    ...
                    if (alarm.isManual) {
                        ...
                        if (alarm.sensorSubType === SdmsResource.sensorSubType.REAL_TIME_ANALYSIS_300330 || alarm.sensorSubType === SdmsResource.sensorSubType.REAL_TIME_ANALYSIS_300331) {
                            realTimeAlarmCount++;
                        }
                        else if (alarm.sensorSubType === SdmsResource.sensorSubType.PREDICTIVE_ANALYSIS) {
                        ...
                        >
                            <header>
                                <div className='eventInfoWrap'>
                                    ...
                                    <p className='sensorTypeName'>{`${SdmsResource.getFacilityTypeString(alarm.facilityType)}`} {(alarm.isSensorAlarm && alarm.sensorSubTypeName) && `(${alarm.sensorSubTypeName})`}</p>
                                </div>
                            ...
                            <section>
                                ...
                                <p>
                                    ...
                                    <span className={isManualReportAlarm ? 'type report' : alarm.isSensorAlarm ? 'type sensor': 'type equipment'}>
                                        {isManualReportAlarm ? '수동신고' : alarm.isSensorAlarm ? '센서탐지' : alarm.sensorSubTypeName}
                                    </span>
                                </p>
                                <p>메모내용<span>{alarm.memo ? alarm.memo : '-'}</span></p>
                            </section>
                            {
                                (selected && alarm.isSensorAlarm && alarm.isAlarm) &&
                                    <footer>
            ...
            setDisplayUI(ui);
            setCount({all: allAlarmCount, sensor: sensorAlarmCount, report: reportAlarmCount, realTimeAnalysis: realTimeAlarmCount, predictiveAnalysis: predictiveAlarmCount});
        };
        ...
        if (menuType === 'sensor') {
            for (const sensorType of props.sensorTypes) {
                if (sensorType.sensorTypeCode === SdmsResource.facilityType.FIRE ||
                    ...
                    sensorType.sensorTypeCode === SdmsResource.facilityType.PM ||
                    sensorType.sensorTypeCode === SdmsResource.facilityType.MOBILE_SCANNER ||
                    sensorType.sensorTypeCode === SdmsResource.facilityType.SUMP ||
            ...
            for (const sensorType of props.sensorTypes) {
                if (sensorType.sensorTypeCode === SdmsResource.facilityType.EQUIPMENT_PredictAlarm ||
                    sensorType.sensorTypeCode === SdmsResource.facilityType.EQUIPMENT_PeakPower
                ) {
                    options.push({ value: sensorType.sensorTypeCode, label: sensorType.sensorTypeName });
                }
            }
    ...
    const onClickClearAlarm = async (e, allSelectedAlarms, alarm) => {
        ...
        if (allSelectedAlarms) {
            props.showConfirmDialog(
                ProjectResource.dialogTypes.MALFUNCTION,
                ["이벤트 알람을 선택 종료하시겠습니까?", `총 ${checkedAlarmList.length} 건의 종료된 이벤트는 되돌릴 수 없으며, 이력 관리 화면에서 확인 가능합니다.`],
                ["취소", "종료하기"],
                clearSelectedAlarms,
                false,
                true
            ...
            props.showConfirmDialog(ProjectResource.dialogTypes.MALFUNCTION, ["이벤트 알람을 종료하시겠습니까?", "종료된 이벤트는 되돌릴 수 없으며, 이력 관리 화면에서 확인 가능합니다."], ["취소", "종료하기"], clearAlarm, alarm.isManual);
        }
    } 
    ...
    const sensorCount = props.sensorAlarm.alarms.filter(a => a.isSensorAlarm).length;
    const equipmentCount = props.sensorAlarm.alarms.filter(a => !a.isSensorAlarm).length;

    const tabs = [
        { key: "sensor", label: "센서", count: sensorCount },
        { key: "equipment", label: "설비", count: equipmentCount },
    ];
                        ...
                        <div>
                            {
                                ...
                                : 
                                <>
                                    <p>실시간 예측 : <span>{count.realTimeAnalysis}</span></p>
                                    <p>예측분석 : <span>{count.predictiveAnalysis}</span></p>
                                </>
                            }
                        </div>
                    </div>
