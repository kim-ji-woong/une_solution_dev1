...
class Contents3D extends Component {
    ...
    constructor(props) {
        ...
        this.refMovingScanners = React.createRef();
        ...
        this.popupMenuLocation = { x: null, y: null };
    }
    ...
    componentDidUpdate(prevProps) {
        ...
        if (prevProps.controlMode !== this.props.controlMode) {
            ...
            // 설비모드 정보 업데이트
            this._3dMaster.facilityManager.setFcltyModel(this._3dMaster, this.props.selFcltyInfo);
        }
        else if (prevProps.fcltyModelName !== this.props.fcltyModelName) {
            // 설비모드 설비 바운딩 박스 선택
            this._3dMaster.facilityManager.selectFcltBoundingBox(this.props.fcltyModelName);
        }

        this._3dMaster.updateScanerInfo(this.props.scannerInfo);
    ...
    onClick = (event) => {
        this.setSimulationPopupMenu(false, 0, 0);
    ...
    getMbPoiIcon = (num) => {
        if (num === 0) return "/resource/image/icon/poi/movingscaner.png";
        else if (num === 1) return "/resource/image/icon/poi/movingscaner_hover.png";
        else if (num === 2) return "/resource/image/icon/poi/movingscaner_selected.png";
        else if (num === 4) return "/resource/image/icon/poi/movingscaner_alarm.png";
        else if (num === 5) return "/resource/image/icon/poi/movingscaner_alarm_hover.png";
        else if (num === 6) return "/resource/image/icon/poi/movingscaner_alarm_selected.png";
        else if (num === 7) return "/resource/image/icon/poi/movingscaner_disabled.png";
        else if (num === 8) return "/resource/image/icon/poi/movingscaner_disabled_hover.png";
        else if (num === 9) return "/resource/image/icon/poi/movingscaner_disabled_selected.png";
    }

    setSimulationPopupMenu(visible, x, y) {
        this.popupMenuLocation.x = x;
        this.popupMenuLocation.y = y;
        this.setState({ expandedCode: visible });
    }

    onKeyDownEditableInput = (event) => {
        if (event.key === 'Enter') {
            this._3dMaster.editModeManager.checkEditingLabel();
            this.props.onChangedEdit(this._3dMaster.editModeManager.isChanged());
            this._3dMaster.editModeManager.resetEditableInput();
        }
    }
    ...
    render() {
        ...
        const editInputID = this.state.editableInput && this.props.isEditMode ? 'areaInput' : 'areaInputHidden';
        ...
        const simBlocks = this.allowedCodes.map((code) => {
            ...
            const codeName = code === this.props.selectedSimulationInfo.buildingGroupName && this.props.selectedSimulationInfo.area.length > 0 ? code + ">" + this.props.selectedSimulationInfo.area : code;

            return (
                <div
                    key={code}
                    id={buildingNo != null ? String(buildingNo) : undefined}
                    className={`simulationBtn`}
                >
        ...
        const mbIcon = this.getMbPoiIcon(0);
        const showMbInfos = true;
        ...
        return (
            <Contents3DComponent>
                ...
                <input
                    ...
                    type="text"
                    id={editInputID}
                    onKeyDown={this.onKeyDownEditableInput}
                />
                ...
                <div ref={this.refMovingScanners} className='mbIcon' hidden>
                    <img src={mbIcon} />
                    {showMbInfos &&
                        <ul>
                            <li>
                                <Icon.CircleCheck size="xxs" fill="state.success" />
                                <p>00명</p>
                            </li>
                            <li>
                                <Icon.CircleX size="xxs" fill="state.error" />
                                <p>00명</p>
                            </li>
                        </ul>
                    }
                ...
                <ul
                    className='toggleBtn'
                    style={{ 
                        display: this.state.expandedCode ? 'block' : 'none',
                        position: 'absolute',
                        top: this.popupMenuLocation.y,
                        left: this.popupMenuLocation.x
                    }}
                >
                    <li onClick={() => this.handleSelectArea("ISO 체결")}>
                        ISO 체결
                    </li>
                    <li onClick={() => this.handleSelectArea("저장 탱크")}>
                        저장 탱크
                    </li>
                </ul>
            </Contents3DComponent>
