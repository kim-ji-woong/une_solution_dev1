...
export class SDMSController {
    static timer = null;    // 알람 관련 타이터
    static alarmTimerCheck = false;
    static emptyAlarmCount = 0;
    static scannerTimer = null;     // 모바일스캐너 관련 타이머
    static scannerTimerCheck = false; 
    ...
    static stopWatchAlarmTimer() {
        ...
        this.emptyAlarmCount = 0;
        
        // store.dispatch({ type: 'SENSOR_ALARM', sensorAlarm: null, sensorAllAlarm: null });
    }

    // 모바일 스캐너 (인가자/비인가자 탐지) 1초에 한번 호출
    static StartScannerTimer() {
        if (this.scannerTimerCheck) return;

        this.scannerTimerCheck = true;

        const scannerTick = async () => {
            try {
                if (!this.scannerTimerCheck) return;
                await SDMSController.WatchScannerInfo();
            } catch (e) {
                console.log("Scanner Timer Error:", e);
            } finally {
                if (this.scannerTimerCheck) {
                    setTimeout(scannerTick, 1000);
                }
            }
        };
    ...
    static stopScannerTimer() {
        this.scannerTimerCheck = false;
    ...
    static async WatchSensorAlarm() {
        ...
        const currentAlarm = store.getState().sensorAlarm ?? [];
        if (result.length === 0 && currentAlarm.length > 0) {
            this.emptyAlarmCount += 1;

            // 일시적인 빈 응답으로 기존 알람 목록이 바로 사라지지 않도록 한 번 더 확인한다.
            if (this.emptyAlarmCount < 2) {
                return;
            }
        }
        else {
    ...
    static async WatchScannerInfo() {
        try {
            const [info, msg] = await SDMSController.requestScannerInfo();
            ...
            const current = store.getState().scannerInfo || null;

            const isChanged = JSON.stringify(current) !== JSON.stringify(info);
            ...
            console.log("WatchScannerInfo Error:", e);
        }
    }
    ...
    static toCompareAlarm(type, result) {
        ...
        // 2) 알람 수가 같고 0이 아닐 때 → 필드 비교
        else if (receiveAlarm.length !== 0) {
            let temp = currentAlarm.slice(); // 깊은 복사
            ...
            isChanged = temp.length !== 0;
        }

        // 변경이 있거나, Redux에 최초 저장이 필요한 경우(sensorAlarm이 null) dispatch
        if (isChanged || store.getState().sensorAlarm == null) {
            store.dispatch({ type: type, sensorAlarm: receiveAlarm, sensorAllAlarm: null });
        }
    }
    ...
    static async requestAdditableSensors(sensorType = null, siteNo = null) {
        try {
            ...
            const res = await fetch(ProjectResource.baseUrl + '/SDMS/SDMS/RequestAdditableSensors', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
    ...
    static async requestTodayAlarmData(siteNo = null, sensorType = null) {
        try {
            ...
            console.log("requestTodayAlarmData Error:", e);
        }
    ...
    static async requestSimulationPoi() {
        try {
            const jsonData = SdmsJsonManager.makeRequestSimulationPoi();

            const res = await fetch('SDMS/SDMS/RequestSimulationPoi', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return [null, "requestSimulationPoi 실패"];
    }
    ...
    static async requestSensorInfo(sensorNo) {
        try {
            ...
            const res = await fetch('SDMS/SDMS/RequesSensorInfo', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
    ...
    static async requestScannerInfo(scannerId) {
        try {
            const jsonData = SdmsJsonManager.makeRequestScannerInfo(scannerId);
            
            const res = await fetch('SDMS/SDMS/RequestScannerInfo', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
            ...
            if (res.ok) {
                const result = await res.json();
                return [result.scanners, result.success, result.message];
            }
        }
        catch (e) {
            console.log(e);
            return [null, false, "requestScannerInfo 실패"];
        }
    }
    
    static async requestScannerTagInfo(sensorZoneHistoryNo) {
        try {
            const jsonData = SdmsJsonManager.makeRequestScannerTagInfo(sensorZoneHistoryNo);
            
            const res = await fetch('SDMS/SDMS/RequestScannerTagInfo', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
            ...
            if (res.ok) {
                const result = await res.json();
                return [result.scannerTag, result.success, result.message];
            }
        } catch (e) {
            console.log(e);
            return [null, false, "requestScannerTagInfo 실패"];
        }
        
        return null;
    }
    
    static async requestScannerEmergencyInfo(sensorZoneHistoryNo) {
        try {
            const jsonData = SdmsJsonManager.makeRequestScannerEmergencyInfo(sensorZoneHistoryNo);
            
            const res = await fetch('SDMS/SDMS/RequestScannerEmergencyInfo', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
            ...
            if (res.ok) {
                const result = await res.json();
                return [result.scannerEmergencyInfo, result.success, result.message];
            }
        } catch (e) {
            console.log(e);
            return [null, false, "requestScannerEmergencyInfo 실패"];
        }
    ...
    static async requestSoulbrainWorkList() {
        try {
            const res = await fetch('http://192.168.254.183:31243/WishData/RequestTodayWorkList', {
                method: 'post',
                headers: {
                    'Content-Type': 'application/json'
                }
