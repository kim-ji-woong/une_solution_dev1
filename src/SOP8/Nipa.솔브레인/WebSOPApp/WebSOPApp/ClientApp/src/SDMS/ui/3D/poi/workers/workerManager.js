...
export class WorkerManager {
    ...
    getTextLayer(siteNo) {
        ...
        if (!buildingGroupTextLayer && !buildingTextLayer && this.baseBuildingGroupTextLayer && this.baseBuildingTextLayer) {
            ...
            if (buildingGroupTextLayer && buildingGroupTextLayer.parent === null) {
                this.baseBuildingGroupTextLayer.add(buildingGroupTextLayer);
            }

            if (buildingTextLayer && buildingTextLayer.parent === null) {
                this.baseBuildingTextLayer.add(buildingTextLayer);
            }
        }
    ...
    clear() {
        ...
        for (const siteNo in siteTextLayers) {
            const textLayer = siteTextLayers[siteNo];
            textLayer.clear();
    ...
    static getBuildingGroupWorker(buildingGroupNo) {
        ...
        if (workers.buildingGroupWorkers) {
            for (const worker of workers.buildingGroupWorkers) {
                if (worker.buld_group_sn === buildingGroupNo) {
    ...
    static getBuildingWorker(buildingNo) {
        ...
        if (workers.buildingWorkers) {
            for (const worker of workers.buildingWorkers) {
                if (worker.buld_sn === buildingNo) {
                    if (worker.wrkr_ty_code === 500500) {
                        currentWorkerCount = worker.wrkr_co;
                    }
                    else if (worker.wrkr_ty_code === 500501) {
    ...
    static getZoneWorker(zoneNo) {
        ...
        if (workers.zoneWorkers) {
            for (const worker of workers.zoneWorkers) {
                if (worker.zone_sn === zoneNo) {
                    if (worker.wrkr_ty_code === 500500) {
                        currentWorkerCount = worker.wrkr_co;
                    }
                    else if (worker.wrkr_ty_code === 500501) {
    ...
    static getEquipZoneWorker(equipZoneNo) {
        ...
        if (workers.equipZoneWorkers) {
            for (const worker of workers.equipZoneWorkers) {
                if (worker.eqp_zone_sn === equipZoneNo) {
                    if (worker.wrkr_ty_code === 500500) {
                        currentWorkerCount = worker.wrkr_co;
                    }
                    else if (worker.wrkr_ty_code === 500501) {
