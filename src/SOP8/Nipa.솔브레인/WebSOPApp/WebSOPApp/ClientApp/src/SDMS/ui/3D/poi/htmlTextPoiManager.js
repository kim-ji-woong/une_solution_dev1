...
export class HtmlTextPoiManager {
    ...
    static MovingScannerNPeople = "movingScannerNPeople";
    ...
    constructor(refBuildingGroupLabels, refBuildingLabels, refEquipZoneLabels, refFacilityTooltip, refFcltyLabels, refMovingScanners, scene, spatialManager, _3dMaster) {
    ...
    makeBillboard(camera) {
        ...
        const movingScannerLayer = this.movingScannerLayer;
        //const movingScannerLayer = this.getTextLayer(HtmlTextPoiManager.MovingScannerNPeople);

        if (movingScannerLayer?.visible) {
            this._makeBillboard(camera, movingScannerLayer);
        }
    ...
    addMovingScannerLabels(sensors, alarmSensors, isIndoor, layer) {
        for (const sensor of sensors) {
            this.addMovingScanerLabel(sensor.sensor, alarmSensors, isIndoor, layer);
        }
    }

    addMovingScanerLabel(sensor, alarmSensors, isIndoor, layer) {
        if (!sensor) {
            return null;
        }
        ...
        if (x !== null && y !== null && z !== null) {
            const clone = this.refMovingScanners.current.cloneNode(true);
            clone.setAttribute('data-movingscanner-clone', sensor.sensor_sn);

            const label = this.setElementPosition(clone, x, y, z);

            if (label) {
                label.element.hidden = false;
                label.name = HtmlTextPoiManager.MovingScannerNPeople + "_" + sensor.sensor_sn;
                label.element.id = label.name;

                const scale = isIndoor ? 0.035 : 0.035 * PoiManager.OutdoorPoiScale / PoiManager.IndoorPoiScale;
                label.scale.set(scale, scale, scale);

                if (alarmSensors && alarmSensors[sensor.sensor_sn]) {
                    this._3dMaster.movingScannerPoiManager.setLabelImage(label, MovingScannerPoiManager.AlarmType);

                    if (this.isSelectedAlarmSensor(sensor.sensor_sn)) {
                        this._3dMaster.movingScannerPoiManager.selectLabel(label);
                    }
                }

                const [permissionCount, nonPermissionCount] = this._3dMaster.getScanerInfo(sensor.sensor_sn);
                this._3dMaster.updateScanerText(label, sensor.sensor_sn, permissionCount, nonPermissionCount);

                this.movingScannerLayer = layer;
                //const layer = this.getTextLayer(HtmlTextPoiManager.MovingScannerNPeople);
                layer.add(label);
                return label;
            }
        }
    ...
    getCfdLabel(buildingName) {
        for (const label of this.cfdTextLayer.children) {
            if (label.userData.originText === buildingName) {
                return label;
            }
        }
            ...
            for (const buildingNo in this.cfdBuildings) {
                ...
                if (div) {
                    ...
                    if (building) {
                        ...
                        label.userData.buildingNo = building.buildingNo;
                        label.userData.originText = building.name;
                        this.cfdTextLayer.add(label);
                    }
                }
            }

            this._3dMaster.props.setTextPoiManager(this);
        }
        else {
    ...
    _changeCfdElementText(label, text, isSelected) {
        if (label.element && label.element.children.length === 2) {
            const btn = label.element.children[1];
            btn.innerText = text;
    ...
    changeCfdElementText(buildingNo, text) {
        if (this.cfdTextLayer) {
            for (const child of this.cfdTextLayer.children) {
                if (buildingNo && child.userData.buildingNo.toString() === buildingNo.toString()) {
                    this._changeCfdElementText(child, text, true);
                }
                else {
                    this._changeCfdElementText(child, child.userData.originText, false);
                }
            }
        }
    }
    ...
    select(x, y, movingScannerPoiManager) {
        ...
        if (textLayer?.visible) {
            for (const child of textLayer.children) {
                if (this._3dMaster.checkParentVisible(child) && this.hitTest(child.element, x, y)) {
                    return child;
                }
            }
        }
        ...
        if (this.movingScannerLayer?.visible) {
            for (const child of this.movingScannerLayer.children) {
                if (child.element) {
                    if (this._3dMaster.checkParentVisible(child) && this.hitTest(child.element, x, y)) {
                        movingScannerPoiManager.selectLabel(child);
                        return child;
                    }
                }
            }
                ...
                if (buildingGroup) {
                    ...
                    if (building) {
                        ...
                        if (fclty) {
                            ...
                            showPopupData[SdmsResource.ID.menu.equipmentChartInfo] = true;
