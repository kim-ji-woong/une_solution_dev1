...
namespace IntegrationServer.Servers.CCTV.S1.SVMS
{
    public class SVMSEventReceiver : IDisposable
    {
        ...
        private int m_serverType = ServerType.CCTV_S1_SVMS;
        ...
        public SVMSEventReceiver(ISVMSEventOwner owner, int nSiteID, string ip, int port, string id, string pw, Dictionary<string, int> dicCCTVSubTypes)
        {
            m_owner = owner;
            m_nSiteID = nSiteID;
            m_strSvmsIP = ip;
        ...
        #region static
        public static List<SVMSEventReceiver> MakeInstances(ISVMSEventOwner owner, DataManager dataManager, int nSiteID, List<string> svmsIPs, List<int> svmsPorts, List<string> svmsIDs, List<string> svmsPWs, Dictionary<string, int> dicCCTVSubTypes)
        {
            ...
            {
                SVMSEventReceiver receiver = new SVMSEventReceiver(owner, nSiteID, svmsIPs[i], svmsPorts[i], svmsIDs[i], svmsPWs[i], dicCCTVSubTypes);
        ...
        public static List<SVMSEventReceiver> CloneInstances(List<SVMSEventReceiver> receivers)
        {
            ...
            {
                SVMSEventReceiver _receiver = new SVMSEventReceiver(receiver.m_owner, receiver.m_nSiteID, receiver.SvmsServerIP, receiver.SvmsPort, receiver.ID, receiver.Password, receiver.DicCCTVSubTypes);
        ...
        private void SVMSEventCameraList(string arg1, bool isSuccess, bool isFinished, List<S1SVMSSDKv2.Model.Device.DeviceCamera> deviceCameras, XmlNode originalActionStructure)
        {
            if (isSuccess == true)
            {
                if (isFinished != true)
                {
                    ...
                    m_owner.OnMessage(DateTime.Now, null, GetSubType(SvmsManager.SubType_NONE), string.Format("CCTV 총 {0}개 읽어오기 완료", m_dicCameras.Count));
                }
                else
                {
