...
namespace IntegrationServer.Servers.IoT.Soulbrain
{
    public class SensorDataManager
    {
        ...
        public static List<string> LoadSensorKeys(DataManager dataManager, int nSensorType, out string strErrorMessage)
        {
        ...
        public static List<AlarmData> GetAlarmList(DataManager dataManager, out string strErrorMessage)
        {
            strErrorMessage = null;
        ...
        public static AlarmData GetAlarmData(DataManager dataManager, DataDevice device, DataSensor sensor, string strSOPWebServerURL, out string strErrorMessage)
        {
            string strUrl = "";
        ...
        /// <returns></returns>
        public static bool UpdateSensorsThresholds(DataManager dataManager, Logger logger, Dictionary<string, DataDevice> dicDevices, out string strErrorMessage)
        {
            strErrorMessage = "";
        ...
        public static void UpdateSensorsThreshold(DataManager dataManager, Logger logger, DataDevice device)
        {
            string strErrorMessage = "";

            if (device == null)
            {
                logger.Write(LogTypes.Error, ServerType.Soulbrain_Hancom, -1, "DataDevice 존재하지 않습니다.");
                return;
            }
            ...
            {
                logger.Write(LogTypes.Error, ServerType.Soulbrain_Hancom, -1, "DataDevice SensorDataList 존재하지 않습니다.");
                return;
            }
            ...
            foreach (DataSensor sensor in listSensorData)
            {
                try
                {
                    ...
                    {   // 생성
                        ...
                        {
                            logger.Write(LogTypes.Error, ServerType.Soulbrain_Hancom, -1, "MaterialLimitData Insert Error: " + strErrorMessage);
                            continue;
                        }
                    }
                    else
                    {   // 업데이트
                        ...
                        {
                            logger.Write(LogTypes.Error, ServerType.Soulbrain_Hancom, -1, "MaterialLimitData Update Error: " + strErrorMessage);
                            continue;
                        }
                    }
                    ...
                    {
                        logger.Write(LogTypes.Error, ServerType.Soulbrain_Hancom, -1, "Material Update Error: " + strErrorMessage);
                        continue;
                    }
                }
                catch (Exception e)
                {
                    logger.Write(LogTypes.Error, ServerType.Soulbrain_Hancom, -1, "UpdateSensorsThreshold Exception: " + e.Message);
                    continue;
                }
            }
        ...
        public static bool UpdateETCSensor(DataManager dataManager, DataDevice device, out string strErrorMessage)
        {
            strErrorMessage = "";
        ...
        public static AlarmData GetAlarmData(DataManager dataManager, AlarmSensorData alarmSensorData, string strSOPWebServerURL, out string strErrorMessage)
        {
            AlarmData alarm = null;
        ...
        public static bool UpdateETCSensor(DataManager dataManager, Logger logger, List<AlarmSensorData> alarmSensors, out string strErrorMessage)
        {
            strErrorMessage = null;
            ...
            foreach (AlarmSensorData alarmSensorData in alarmSensors)
            {
                ...
                foreach (DataSensor sensor in listSensorData)
                {
                    ...
                    try
                    {
                        ...
                        if (sensorZone == null)
                        {
                            logger.Write(LogTypes.Error, ServerType.Soulbrain_Hancom, -1, $"UpdateETCSensor SelectFirst : {alarmSensorData.UniqueKey}의 SensorZone 조회되지 않습니다.");
                            continue;
                        }
                        ...
                        if (dataManager.GetUpdate().Update<Material, Material.Fields>(dicSets, strConditions, out strErrorMessage) == false)
                        {
                            logger.Write(LogTypes.Error, ServerType.Soulbrain_Hancom, -1, "UpdateETCSensor Update : " + strErrorMessage);
                            continue;
                        }
                    }
                    catch (Exception e)
                    {                        
                        logger.Write(LogTypes.Error, ServerType.Soulbrain_Hancom, -1, "UpdateETCSensor Exception : " + e.Message);
                    }
                }
            }
