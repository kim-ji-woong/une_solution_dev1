...
function SopSimulator(props) {
    ...
    useEffect(() => {
        ...
        // 타이틀바 클릭 이벤트 핸들러
        props.menuEvent.handler = onClickLogo;
    ...
    const loadSOP = async (storeValue) => {
        ...
        if (selectedSiteNo === null) {            
            if (userInfo) {
                ...
                if (userInfo.levelNo === AccountResource.accountLevelID.master) {
                    if (ProjectResource.sites.length > 0 && userInfo.levelNo === AccountResource.accountLevelID.master) {
                        newSelectedSiteNo = -1;
                    }
                    else {
                    ...
                    newSelectedSiteNo = userInfo.siteNo;
                }
            }
        }
                        
        const sopHistories = checkAlreadySOP(storeValue.sopHistory.sopRunDatas);
        ...
        if (sopHistories === null || sopHistories.length === 0) {
            ...
            let sopTabIndex = 0;
            let newCurrentSopTabKey = currentSopTabKey;
            let newCurrentActionStep = currentActionStep;
            ...
            for (let i = 0; i < historyLength; i++) {
                ...
                if (userInfo.levelNo !== AccountResource.accountLevelID.master && userInfo.siteNo !== sopHistory.siteNo) {
                    continue;
                }

                // 각 SOP의 현재 단계 설정
                //const currentActionStepID = getCurrentActionStepID(sopHistory.key);
                //setCurrentActionStep(sopHistory.sopData, currentActionStepID, lastAccessActionStepHistoryID);
                setCurrentActionStep2(sopHistory.sopData, lastAccessActionStepHistoryID);

                checkArrows(sopHistory.sopData);
                await checkStepMembers(sopHistory.sopData);

                allSopHistoriesDump.push(sopHistory);

                // 모든 SOP를 볼수있는 권한이고, 지금 선택된 SITE의 SOP일때
                if (newSelectedSiteNo === -1 || newSelectedSiteNo === sopHistory.siteNo) {
            ...
            for (let i = 0; i < historyDumpLength; i++) {
                ...
                if (optionIndex === 1 && currentSopTabKey.length > 0 && sopHistory.key === currentSopTabKey) {                    
                    sopTabIndex = i;
                    newCurrentSopTabKey = currentSopTabKey;
                    ...
                    for (let j = 0; j < sopHistory.sopData.actionStepDatas.length; j++) {
                        ...
                        newCurrentActionStep = sopHistory.sopData.actionStepDatas[j];
                        currentSopData = sopHistory.sopData;
            ...
            if (closeSOPs.length > 0) {
                ...
                for (let i = 0; i < historyLength; i++) {
                    ...
                    for (let j = 0; j < sopHistory.sopData.actionStepDatas.length; j++) {
                        ...
                        if (sopHistory.sopData.actionStepDatas[j]._ActionStepHistory.id === closeSOPHistoryID) {
                            ...
                            newCurrentActionStep = sopHistory.sopData.actionStepDatas[j];
                            currentSopData = sopHistory.sopData;
                            confirmDialogData = sopHistory.sopData.actionStepDatas[j]._ActionStepHistory;
                            break;
                        }
                    }
                }
                ...
                if (confirmDialogData) {
                    ...
                    if (confirmDialogData.sensorZoneHistoryID && confirmDialogData.sensorZoneHistoryID > 0) {
                        message = i18n.t('sopSimulator.formText.신호 복구 후 자동 종료시간이 경과되어 SOP를 종료합니다');
                    }
                    showConfirmDialog(i18n.t('sopSimulator.formText.알림'), message, [i18n.t('common.확인')], confirmDialogCloseSOP);
                }
            }

            // flow chart 설정
            //const currentActionStepID = getCurrentActionStepID(currentSopTabKey);
            //if (currentSopData && currentSopData !== null) {
            //    setCurrentActionStep(currentSopData, currentActionStepID, lastAccessActionStepHistoryID);

            //    checkArrows(currentSopData);
            //    await checkStepMembers(currentSopData);
            //}
    ...
    const checkAlreadySOP = (sopRunDatas) => {
        const oldSopDatas = viewSopDatas;
    ...
    const openDB = async (versionID, alarm) => {        
        ...
        if (sopDataResult && sopDataResult.success) {
            ...
            if (sopTabIndex > -1) {
                ...
                newSOP.siteNo = sopDataResult.sopData.version.siteNo;// selectedSiteNo;
    ...
    const getCurrentActionStepID = (sopKey) => {
        const orgSopDatas = viewSopDatas;
        if (!orgSopDatas) {
            return -1;
        }
    ...
    const setCurrentActionStep2 = (sopData, lastActionStepHistoryID) => {
    ...
    const confirmDialogCloseSOP = async (index) => {        
        ...
        if (actionStepHistory) {
            ...
            await closeSOP(null, actionStepHistory.id, actionStepHistory.endTime, null);
        }
        confirmDialogData = undefined;
        onCloseConfirmDialog();
    }

    const closeSOP = async (tabIndex, actionStepHistoryID, endTime, accessedUserID) => {
    ...
    const onChangeTab = (index) => {
        ...
        setCurrentActionStep(sopDatas[index].sopData.currentActionStep);
    }
    ...
    const onChangeStep = async (stepID, stepName) => {        
        ...
        const sopData = newSopDatas[sopTabIndex].sopData;
    ...
    const onClickLogo = () => {
        onSelectMenu(SopSimulator.resource.ID.menu.callSOP, null);
    }
