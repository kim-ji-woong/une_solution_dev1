...
import { SopSimulatorSBChartComponent } from '../styled/sopSimulatorStyled';

function SopSimulatorSBChart(props) {
    ...
    useEffect(() => {
        ...
        if (prevProps.current?.currentSection !== props.currentSection) {
            ...
            if (props.currentSection?.length > 0) {
                ...
                sectionData = props.currentSection[lastIndex];
            }
            ...
            // 실행중인 컴포넌트로 자동 화면 이동
            if (props.commonSettings.UseAutoMoveSOPScreen && sectionData) {
                gridCenterForcus(sectionData?.gridRowIndex, sectionData?.gridColumnIndex);
            }
        }

        prevProps.current = props;

    }, [props.sopData, props.sopTabIndex, props.currentSection, firstUpdateChk, props.commonSettings.UseAutoMoveSOPScreen]);
    ...
    const toOriginalLocation = () => {
        if (props.currentSection?.length > 0) {
            const lastIndex = props.currentSection.length - 1;
            const sectionData = props.currentSection[lastIndex];
    ...
    const getSOPName = () => {
        let sopName = '';
        if (props.sopData?.disaster?.disasterName) {
            sopName = props.sopData.disaster.disasterName + ' ';

            if (props.sopData.disaster.version) {
                //const mode = sopData.disaster.version.isNormal ? "(" + SopManagerResource.ID.sopMode.day + ")" : "(" + SopManagerResource.ID.sopMode.night + ")";
                //return sopName + mode;
            }
        }
    ...
    const showConfirmDialog = () => {
        ...
        if(userInfo && userInfo !== null && (parseInt(userInfo.levelNo) === AccountResource.accountLevelID.master || parseInt(userInfo.levelNo) === AccountResource.accountLevelID.generalAdmin)) {
    ...
    const onClickEditSOP = (index) => {        
        ...
        if (index <= 1) {
            props.history.push("/sop-manager?sop=" + props.sopData.version.id, '_blank');
            // window.open("/sop-manager?sop=" + props.sopData.version.id, '_blank');
        }
    ...
    return (
        <>
            {/*soulbrainchart*/}
            <SopSimulatorSBChartComponent className={'subSection progressViewWrap'}>
