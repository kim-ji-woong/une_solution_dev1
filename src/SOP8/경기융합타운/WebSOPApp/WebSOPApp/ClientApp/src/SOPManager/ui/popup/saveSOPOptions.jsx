...
function SaveSOPOptions(props) {
	...
	const getVersions = async (sopData) => {
		...
		const [disasterVersions, message] = await SopController.requestDisasterVersions(sopData, isNormal);
	...
	const onClickApply = (ok) => {
		if (ok) {
			if (refCheckNewVersion.current.checked) {
				...
				sopData.version = SopDataManager.makeNewVersion(isNormal, newVersionName, props.loginUser ? props.loginUser.id : -1, refTextNewVersionDescription.current.value.toString().trim());
				props.content(SopManagerResource.menu.save, sopData);
				return;
			}
	
			if (selectedVersion) {
				sopData.version.id = selectedVersion.id;
										...
										<div className={'spprCont'}>
											<table className={'scTb'}>
												...
												<tbody>
													{
														currentVersion && (
															<tr onClick={(event) => tbRdo(event, currentVersion)}>
																<td>{currentVersion.versionName}</td>
																<td>{currentVersion.owner}</td>
																<td>{currentVersion.createTime.replace('T', ' ')}</td>
																<td>{currentVersion.lastAccessTime.replace('T', ' ')}</td>
																<td>{currentVersion.description}</td>
															</tr>
														)
													}
