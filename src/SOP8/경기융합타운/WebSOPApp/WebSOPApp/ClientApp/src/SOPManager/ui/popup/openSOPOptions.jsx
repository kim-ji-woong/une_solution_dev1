...
function OpenSOPOptions(props) {
	...
	const onClickOpen = (event) => {
		if (selectedVersion) {
			props.content(SopManagerResource.menu.open, selectedVersion.id);
		}
		else {
		...
		if (disaster?.version) {
			...
			return (
				<tr key={"version_" + versionCount} onClick={(event) => tbRdo(event, disaster.version)}>
					<td>
						<span
							onMouseOver={(e) => props.handleTooltip(e, disaster.version.versionName)}
					...
					<td className={'tal'}>
						<span
							onMouseOver={(e) => props.handleTooltip(e, disaster.version.description)}
							onMouseLeave={() => props.removeTooltip()}
						>
	...
	const getSubDisasterCategoryContents = (subDisasterCategoryData) => {
		...
		if (subDisasterCategoryData.disasterDatas && subDisasterCategoryData.disasterDatas.length > 0) {
			return (
				<li key={"sdc_" + sdcCount}>
					<i className="fa-minus" onClick={onClickTreeNode}>더보기</i><h5>{subDisasterCategoryData.subDisasterCategory.subCategoryName}</h5>
					{
		...
		return (
			<li key={"sdc_" + sdcCount} className={"treeviewLastItem " + " " + 'grayText'}>{subDisasterCategoryData.subDisasterCategory.subCategoryName}</li>
		);
	}
	...
	const getDisasterCategoryContents = (disasterCategoryData) => {
		...
		return (
			<li key={"dc_" + dcCount}>
				<i className="fa-minus" onClick={onClickTreeNode}>더보기</i><h5>{disasterCategoryData.disasterCategory.categoryName}</h5>
				{
							...
							<div className={'sppCont'}>
								...
								<div className={'sppRht'}>
									...
									<div className={'spprBot'}>
										<a className={'blu'} onClick={onClickOpen}>SOP 열기</a>
									</div>
								</div>
							</div>
						</div>
