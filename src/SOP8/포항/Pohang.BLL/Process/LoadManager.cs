    ...
    public class LoadManager
    {
        ...
        public ResponseExternalMaterialLinks GetExternalMaterialLinks()
        {
            ...
            foreach (MaterialLink materialLink in materialLinks)
                response.MaterialLinks.Add(materialLink);
        ...
        public ResponseExternalPublicData GetExternalPublicData()
        {
            ResponseExternalPublicData response = new ResponseExternalPublicData();
            
            IEnumerable<PublicTms> publicTmsList = m_dataManager.GetSelect().Select<PublicTms>(null, out string strErrorMessage);
            
            if (publicTmsList == null)
                return new ResponseExternalPublicData(false, strErrorMessage);
            
            List<PublicTms> publicTms = publicTmsList.ToList();
            
            IEnumerable<PublicWeather> publicWeatherList = m_dataManager.GetSelect().Select<PublicWeather>(null, out strErrorMessage);
            
            if (publicWeatherList == null)
                return new ResponseExternalPublicData(false, strErrorMessage);
            
            List<PublicWeather> publicWeather = publicWeatherList.ToList();
            
            IEnumerable<PublicAirKorea> publicAirKoreaList = m_dataManager.GetSelect().Select<PublicAirKorea>(null, out strErrorMessage);
            
            if (publicAirKoreaList == null)
                return new ResponseExternalPublicData(false, strErrorMessage);
            
            List<PublicAirKorea> publicAirKorea = publicAirKoreaList.ToList();
            
            response.PublicTmsList = publicTms;
            response.PublicWeatherList = publicWeather;
            response.PublicAirKoreaList = publicAirKorea;
            response.Success = true;
            return response;
        }
    }
