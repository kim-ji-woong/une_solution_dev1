...
class SensorSimulator extends Component {
    ...
    isAlarmSensor = (sensor) => {
        ...
        return !this.isNoneAlarmTypeCode(sensor.sensor.sensor_ty_code);
    } 
    
    isAlarmMaterial = (sensor) => {
        if (sensor.sensor.subTypeName === "Temp" ||
    ...
    onClickSendAlarm = async (isAlarm, allClear) => {
        if (isAlarm) {
            ...
            if (!selectedSensor) {
                this.showConfirmDialog(ProjectResource.dialogTypes.INFO, ["센서를 선택해주세요."], null, null);
            }
            else {
