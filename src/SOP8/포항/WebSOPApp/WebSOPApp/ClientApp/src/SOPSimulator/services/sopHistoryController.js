...
export class SopHistoryController {
    ...
    static async requestConvertSpecialCharacters(actionStepHistoryID, message) {
        try {
            const jsonData = SopSimulatorJsonManager.makeRequestConvertSpecialCharacters(actionStepHistoryID, message);

            const response = await fetch('api/Pohang/RequestConvertSpecialCharacters', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return [null, "requestConvertSpecialCharacters 실패"];
    }
}