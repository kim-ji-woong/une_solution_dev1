...
import {SopHistoryController} from "../services/sopHistoryController";
...
function SopSimulatorBody(props) {
    ...
    const runSectionFromChart = (selectSection) => {
        ...
        if (currentActionStepHistoryNo > 0 && selectSection.component.compn_code === 3 && selectSection.endpoint?.begin_yn) {
            // SOP 시작했는데 시작section 눌렀을 때
            return false;
        }

        if (selectSection.component.compn_code === 3 && selectSection.endpoint?.begin_yn) {
            // 시작한 경우 다음 세션을 찾아야하므로 runSection 함수에 넘긴다
            runSection(selectSection);
        }
        else if (selectSection.component.compn_code === 3 && !selectSection.endpoint?.begin_yn) {
            // 종료 컴포넌트 선택시 종료
            closeSOP(false);
        }
        else {
    ...
    const runSection = async (section, decisionValue, fromChart) => { 
        if (section.component.compn_code === 3 && !section.endpoint.begin_yn) {
            // 종료 컴포넌트 선택시 종료
            closeSOP(false);
            return;
        }
    ...
    const onProgressSpread = async (section, dataIndex, useSMS, useEmail, useBroadcast, useSiren, message) => {
        ...
        message = await SopHistoryController.requestConvertSpecialCharacters(actionStepHistoryNo, message);

        const [success, responseMessage] = await SopSimulatorController.sendMessage(actionStepHistoryNo, section.component.compn_sn, section.component.compn_code, userNo, useSMS, useEmail, useBroadcast, useSiren, message, receivers, siteNo);
    ...
    const beginSopData = () => {
        if (currentSection) {
            if (currentSection.component.compn_code === 3 && currentSection.endpoint.begin_yn) {
    ...
    const closeSOP = async (ignoreSummary) => {
        ...
        if (actionStepHistory && actionStepHistory !== null) {
            ...
            // SOP 결과 요약창 사용?
            if (props.commonSettings?.UseSopSummary?.value === 'true' && !ignoreSummary) {
        ...
        <>       
            <SopSimulatorBodyComponent className={'appContainerWrapSop paddingTop60' + " " + 'UI_Section'}>
                <div className={'appContainer pgProgress'}>
                    ...
                    <ProcessList 
                        sopRunData={props.sopDatas[props.sopTabIndex]} 
