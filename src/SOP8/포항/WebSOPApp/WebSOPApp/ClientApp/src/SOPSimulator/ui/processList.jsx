...
function ProcessList(props) {
                ...
                for (let k = 0; k < sectionLength; k++) {
                    ...
                    if (section.component.compn_sn === history.compn_sn && section.component.compn_code === history.compn_code) {
                        ...
                        if ((history.sop_sttus_code - history.sop_sttus_optn_code) === 3) {
                            ...
                            if (section.component.compn_code === 0) {
                                // 프로세스
                                if (section.checked) {
                                    value.status = "완료";
                                }
                            ...
                            else if (section.component.compn_code === 6) {
                                if (section.checked) {
                                    value.status = "완료";
                                }
                            }
                        ...
                        if (section.component.compn_code === 0 || section.component.compn_code === 6) {
                            // if (historyDetail) {
                            //     if (!detail) {
                            //         detail = [];
                            //     }
    ...
    const getSubSection = () => {
        ...
        if (props.sopRunData) {
            ...
            const formatedBeginTime = beginTime.split('.')[0];
            
            ui.push(
                <ul key={position}>
                    <li>위치 : {position}</li>
                    <li>발생시간 : {formatedBeginTime.length > 0 ? formatedBeginTime : 'N/A'}</li>
                    <li>감지센서 : {props.sensorName}</li>
                    <li>유형 : {type}</li>
                    <li>단계 : {stepName}</li>
                </ul>
            );
