        ...
        const sensorTick = async () => {
            try {
                ...
                if (this.sensorTimerCheck) {
                    this.sensorTimer = setTimeout(sensorTick, 2000);
                }
            }
        };
        sensorTick();
    ...
    static MakeSensorList = (allSensors, st, slk) => {
        ...
        for (let i = 0; i < sensorLinks.length; i++) {
            ...
            if (targetSensorType) {
                ...
                let filteredSensors = [];
                // sensor_type_idx가 3, 6, 7, 10, 11이면 CCTV
                if(sensorLink.sensor_type_idx === SdmsResource.externalSensorType.waterLevelCCTV ||
                   sensorLink.sensor_type_idx === SdmsResource.externalSensorType.environmentCCTV ||
                   sensorLink.sensor_type_idx === SdmsResource.externalSensorType.mainStreetCCTV ||
                   sensorLink.sensor_type_idx === SdmsResource.externalSensorType.smartCrosswalkCCTV ||
                   sensorLink.sensor_type_idx === SdmsResource.externalSensorType.smartCrosswalkLightingCCTV ||
                   sensorLink.sensor_type_idx === SdmsResource.externalSensorType.smartCrossroad) 
                {
