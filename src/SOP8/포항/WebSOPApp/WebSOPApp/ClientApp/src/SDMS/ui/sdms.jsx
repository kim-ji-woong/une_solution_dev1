...
import {SdmsStyled} from "../styled/sdmsPopupsStyled";
...
import PublicDataInfo from './popups/publicDataInfo';
...
function SDMS(props) {
    ...
    const [visibleSensorTypes, setVisibleSensorTypes] = useState({});
    ...
    useEffect(() => {
        ...
        const unsubscribeSettings = SettingsStore.subscribe(() => {
            ...
            if (data.actionType === 'COMMON_SETTINGS') {
                ...
                const useAlarmSound = data.commonSettings.find(c => c.categoryType === "SDMS")?.settingDatas.find(o => o.name === "UseAlarmSound")?.value || true;
    ...
    useEffect(() => {
        if (!sensorList || sensorList.length === 0 || !selectedSensor.sensor) {
            return;
        }

        const currentSensorNodeId = selectedSensor.sensor.sensorLink.node_id;

        // 새로운 sensorList 구조 안에서 현재 선택된 node_id와 같은 센서(Zone) 객체를 찾음
        // sensorList -> sensorType -> zones -> target Zone
        let updatedSensorObj = null;

        for (const type of sensorList) {
            if (type.zones) {
                const foundZone = type.zones.find(z => z.sensorLink.node_id === currentSensorNodeId);
                if (foundZone) {
                    updatedSensorObj = foundZone;
                    break;
                }
            }
        }

        // 찾은 최신 객체가 현재 들고 있는 객체와 다르다면 상태 업데이트
        if (updatedSensorObj && updatedSensorObj !== selectedSensor.sensor) {
            setSelectedSensor(prev => ({
                ...prev,
                sensor: updatedSensorObj
            }));
        }

    }, [sensorList]); // sensorList가 변할 때마다 실행
    ...
    const setMaterialTypeToAlarms = (alarms) => {
        ...
        alarms.forEach(alarm => {
            ...
            alarm.detectType = targetSensorType.sensor_category_idx;
            alarm.detectTypeString = SDMSResource.getDetectTypeString(targetSensorType.sensor_category_idx);
        });
    }
    ...
    const getAlarmSound = () => {
        if (useAlarmSound === "FALSE" || !selectedAlarm?.isAlarm) {
            return <></>;
        }
    ...
    const getPopupUI = () => {
        ...
        if (showPopups[menu.publicData]) {
            popups.push(
                <PublicDataInfo key='sdms_popup_publicData'
    ...
    return (
        ...
        <div>
            <Contents3D />
            {
                (!isSimulationMode && showEventDashboard) &&
                    <EventDashboard
