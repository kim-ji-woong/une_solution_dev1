...
function StatusSensorInfo(props) {
    ...
    const getCriticalMass = () => {
        ...
        for (let i = 0; i < props.selectedSensor?.sensors.length; i++) {
            ...
            targetMaterialLink = materialLink;
        ...
        criticalMassContent.push(
            <div id='toolTipContent' key={"toolTipContent" + "_" + targetMaterialLink?.sensor_name_kor}>
                <ul>
    ...
    const getSensorDatas = () => {
        ...
        for (const data of datas) {
            ...
            if (isNoneWeatherMaterial(data.sensorZoneData.sensorZone.sensor_sub_ty_no)) {
                const status = getThreshold(data);
                
                ui.push(
                    <li className={data.status === 4 ? 'warning' : null} key={data.sensorZoneData.sensorZone.sensor_sub_ty_no}>
                        <div><span>{sensorTypeName}</span></div>
                        <div><span>{data.sensorZoneData.sensorMaterial.cur_data ? data.sensorZoneData.sensorMaterial.cur_data : '-'}</span></div>
                        <div>
                            <div className={'chartArea'}>
                                ...
                                <div className={'chartAnimate' + status}></div>
                            </div>
                        </div>
                    </li>
                );
    ...
    const isNoneWeatherMaterial = (sensorSubTyNo) => {
        ...
        if (targetSubType) {
            ...
            return !(targetSubTypeName === "Temp" || targetSubTypeName === "Humi" || targetSubTypeName === "WD" || targetSubTypeName === "WS");
        }
    ...
    const getWeatherDatas = () => {
        ...
        for (const data of datas) {
            if (["Temp", "Humi", "WD", "WS"].includes(data.sensorZoneData.sensorZone.unq_key.split('_')[2])) {
                ui.push(<p key={data.sensorZoneData.sensorZone.sensor_sub_ty_no}>{data.sensorZoneData.sensorMaterial.cur_data ? data.sensorZoneData.sensorMaterial.cur_data : '-'}</p>);
            }
        }
            ...
            >
                <div className='dslTop'>
                    <h5 className='dslTitle'>
                        {SdmsResource.ID.menu.statusSensorInfo}
                    </h5>
                    <input
                        type="range"
                    ...
                    />
                    <button className='dslX' onClick={() => onClickClosePopup(SdmsResource.ID.menu.statusSensorInfo, false)}>닫기</button>
                </div>
