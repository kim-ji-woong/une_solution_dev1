...
{
    class FireSensorManager
    {
        private const int AddedSensorNumber = 0;
        private IMigrationClient m_client = null;
        private int m_nSop8SiteNo = -1;

        public FireSensorManager(IMigrationClient client, int sop8SiteNo)
        {
            m_client = client;
        ...
        public bool Run()
        {
            m_client.SendStatus("화재센서 데이터를 읽어옵니다.");
            ...
            m_client.SendStatus("화재센서 데이터를 옮기는데 성공하였습니다.");
            return true;
        }
        ...
        private bool ReadSop7(out string strErrorMessage)
        {
            ...
            string strSQL = "Select ID, Name, PositionName, X, Y,Z, ZoneID, Department, DepartmentPhoneNumber, Enabled, SensorSubType from SdmsSensorFire";
        ...
        {
            string strSQL = "Select a.ID ID, a.SensorType SensorType, a.OrgSensorID OrgSensorID, a.EquipZoneID EquipZoneID, a.IsAlarmStatus IsAlarmStatus, a.Data Data, b.SensorServerID SensorServerID, b.TagNo TagNo, b.Activate Activate ";
            strSQL += "from SdmsSensorZone a inner join SdmsSensorTagInfo b on a.ID = b.SensorZoneID and a.SensorType = 0";
            IEnumerable<dynamic> arrResults = m_client.Sop7DataManager.GetSelect().Select(strSQL, out strErrorMessage);
        ...
        private bool CreateSop8(dynamic data, Dictionary<int, dynamic> dicSensorZoneDatas, out string strErrorMessage)
        {
            ...
            sensor.sensor_ty_code = SdmsSensor.SensorType.Fire;
        ...
        private bool CreateSensorZone(Base.Model.Sensor.Sensor sensor, dynamic sensorZoneData, dynamic sensorData, out string strErrorMessage)
        {
            ...
            sensorZone.sensor_sub_ty_no = sensorData.SensorSubType;
            sensorZone.unq_key = sensorZoneData.TagNo.ToString();
            ...
            return m_client.Sop8DataManager.GetCreate().Insert<SensorZone>(sensorZone, out strErrorMessage);
        }
