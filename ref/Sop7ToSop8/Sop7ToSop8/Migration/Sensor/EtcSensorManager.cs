...
{
    class EtcSensorManager
    {
        private const int AddedSensorNumber = 210000;
        private IMigrationClient m_client = null;
        private int m_nSop8SiteNo = -1;

        public EtcSensorManager(IMigrationClient client, int sop8SiteNo)
        {
            m_client = client;
        ...
        public bool Run()
        {
            m_client.SendStatus("기타센서 데이터를 읽어옵니다.");
            ...
            m_client.SendStatus("기타센서 데이터를 옮기는데 성공하였습니다.");
            return true;
        }
        ...
        private bool ReadSop7(out string strErrorMessage)
        {
            ...
            string strSQL = "Select ID, Name, PositionName, X, Y, Z, CurrentData, ZoneID, Department, DepartmentPhoneNumber, Enabled, Status, UniqueKey, MaterialType, LimitBase, LimitType, LimitValue from SdmsSensorETC";
        ...
        private Dictionary<int, dynamic> ReadSop7SensorZones(out string strErrorMessage)
        {
            ...
            strSQL += "from SdmsSensorZone a inner join SdmsSensorTagInfo b on a.ID = b.SensorZoneID and a.SensorType = 21 inner join SdmsSensorETC c on a.OrgSensorID = c.ID";
            IEnumerable<dynamic> arrResults = m_client.Sop7DataManager.GetSelect().Select(strSQL, out strErrorMessage);
        ...
        private bool CreateSop8(dynamic data, Dictionary<int, dynamic> dicSensorZoneDatas, out string strErrorMessage)
        {
            ...
            sensor.sensor_ty_code = SdmsSensor.SensorType.Etc;
