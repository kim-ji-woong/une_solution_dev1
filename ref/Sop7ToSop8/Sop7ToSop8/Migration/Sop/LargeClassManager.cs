...
namespace Sop7ToSop8.Migration.Sop
{
    ...
    class LargeClassManager
    {
        private IMigrationClient m_client = null;
        private int m_nSop8SiteNo = -1;

        public LargeClassManager(IMigrationClient client, int sop8SiteNo)
        {
            m_client = client;
        ...
        public bool Run()
        {
            m_client.SendStatus("SOP 대분류 데이터를 읽어옵니다.");
            ...
            m_client.SendStatus("SOP 대분류 데이터를 옮기는데 성공하였습니다.");
            return true;
        }

        private bool ReadSop7(IDataManager dataManager, out string strErrorMessage)
        {
            string strSQL = "Select ID, CategoryName from SopCategoryDisasterCategory";
            ...
            // 자동증가 초기화
            if (dataManager.GetUpdate().Update("DBCC CHECKIDENT(" + LargeClass.TableName + ", reseed, 0)", out strErrorMessage) == false)
                return false;

            // 자동증가 해제
            if (dataManager.GetUpdate().Update("SET IDENTITY_INSERT " + LargeClass.TableName + " ON", out strErrorMessage) == false)
                return false;
            ...
            foreach (var data in arrResults)
            {
                if (CreateSop8(dataManager, data, out strErrorMessage) == false)
                {
                    // 자동증가 설정
                    string strTemp;
                    dataManager.GetUpdate().Update("SET IDENTITY_INSERT " + LargeClass.TableName + " OFF", out strTemp);
                    return false;
                }
            }

            // 자동증가 설정
            if (dataManager.GetUpdate().Update("SET IDENTITY_INSERT " + LargeClass.TableName + " OFF", out strErrorMessage) == false)
                return false;
        ...
        {
            LargeClassEx model = new LargeClassEx();
            model.lclas_sn = data.ID;
            model.lclas_name = data.CategoryName;
            model.site_sn = m_nSop8SiteNo;

            return dataManager.GetCreate().Insert<LargeClassEx>(model, out strErrorMessage);
        }
    }
}
