...
namespace Sop7ToSop8.Migration.Sop
{
    ...
    {
        private const int AddNumber = 20000;
        private IMigrationClient m_client = null;
        private int m_nSop8SiteNo = -1;

        public DecisionManager(IMigrationClient client, int sop8SiteNo)
        {
            m_client = client;
        ...
        public bool Run()
        {
            m_client.SendStatus("SOP 판단 데이터를 읽어옵니다.");
            ...
            m_client.SendStatus("SOP 판단 데이터를 옮기는데 성공하였습니다.");
            return true;
        }

        private bool ReadSop7(IDataManager dataManager, out string strErrorMessage)
        {
            string strSQL = "Select ID, GridID, GridRowIndex, GridColumnIndex, text, TeamID, TeamType, ComponentID, StepMemberID, autoRunScript, autoRunScriptVariableTypes, SectionNumber, Description from SopComponentDecision";
        ...
        {
            if (ComponentManager.CreateComponent(dataManager, dnsData.CommonCode.Sop.ComponentType.Decision, AddNumber, data, out strErrorMessage) == false)
                return false;

            Decision section = new Decision();
            section.compn_sn = data.ID + AddNumber;
            section.title = data.text;
            section.descp = data.Description;
            section.atmc_execut_script = data.autoRunScript;
            section.execut_no = data.SectionNumber;

            return dataManager.GetCreate().Insert<Decision>(section, out strErrorMessage);
        }
