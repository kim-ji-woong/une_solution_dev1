...
namespace Sop7ToSop8.Migration.Team
{
    ...
    class RegularTeamManager
    {
        private IMigrationClient m_client = null;
        private int m_nSop8SiteNo = -1;

        public RegularTeamManager(IMigrationClient client, int sop8SiteNo)
        {
            m_client = client;
        ...
        public bool Run()
        {
            m_client.SendStatus("정규조직 데이터를 읽어옵니다.");
            ...
            m_client.SendStatus("정규조직 데이터를 옮기는데 성공하였습니다.");
            return true;
        }
        ...
        private bool ReadSop7(IDataManager dataManager, out string strErrorMessage)
        {
            ...
            // 자동증가 초기화
            if (dataManager.GetUpdate().Update("DBCC CHECKIDENT(" + Regular.TableName + ", reseed, 0)", out strErrorMessage) == false)
                return false;

            // 자동증가 해제
            if (dataManager.GetUpdate().Update("SET IDENTITY_INSERT " + Regular.TableName + " ON", out strErrorMessage) == false)
                return false;
            ...
            foreach (var data in arrResults)
            {
                if (CreateSop8(dataManager, data, out strErrorMessage) == false)
                {
                    // 자동증가 설정
                    string strTemp;
                    dataManager.GetUpdate().Update("SET IDENTITY_INSERT " + Regular.TableName + " OFF", out strTemp);
                    return false;
                }
            }

            // 자동증가 설정
            if (dataManager.GetUpdate().Update("SET IDENTITY_INSERT " + Regular.TableName + " OFF", out strErrorMessage) == false)
                return false;
        ...
        {
            RegularEx regular = new RegularEx();
            regular.rgl_sn = data.ID;
            regular.team_name = data.TeamName;
            regular.parnts_sn = data.ParentTeamID;
            regular.site_sn = m_nSop8SiteNo;

            return dataManager.GetCreate().Insert<RegularEx>(regular, out strErrorMessage);
        }
    }
}
