...
namespace Sop7ToSop8.Migration.Account
{
    ...
    class UserManager
    {
        ...
        public UserManager(IMigrationClient client, int sop8SiteNo)
        {
            m_client = client;
        ...
        public bool Run()
        {
            m_client.SendStatus("계정 데이터를 읽어옵니다.");
            ...
            m_client.SendStatus("계정 데이터를 옮기는데 성공하였습니다.");
            return true;
        }

        private bool ReadSop7(IDataManager dataManager, out string strErrorMessage)
        {
            string strSQL = "Select ID, MemberID, UserLevel, Password, UserID, NickName, SiteID, PasswordCode, Salt from SopAccountUser";
            ...
            // 자동증가 초기화
            if (dataManager.GetUpdate().Update("DBCC CHECKIDENT(" + User.TableName + ", reseed, 0)", out strErrorMessage) == false)
                return false;

            // 자동증가 해제
            if (dataManager.GetUpdate().Update("SET IDENTITY_INSERT " + User.TableName + " ON", out strErrorMessage) == false)
                return false;
            ...
            foreach (var data in arrResults)
            {
                if (CreateSop8(dataManager, data, out strErrorMessage) == false)
                {
                    // 자동증가 설정
                    string strTemp;
                    dataManager.GetUpdate().Update("SET IDENTITY_INSERT " + User.TableName + " OFF", out strTemp);
                    return false;
                }
            }

            // 자동증가 설정
            if (dataManager.GetUpdate().Update("SET IDENTITY_INSERT " + User.TableName + " OFF", out strErrorMessage) == false)
                return false;
        ...
        private bool CreateSop8(IDataManager dataManager, dynamic data, out string strErrorMessage)
        {
            ...
            user.user_sn = data.ID;
            user.grad_sn = data.UserLevel;
            user.rgl_memb_sn = data.MemberID;
            user.password = data.Password;
            user.user_id = data.UserID;
            user.user_name = data.NickName;
            user.site_sn = m_nSop8SiteNo;
            user.password_key = data.PasswordCode;
            user.password_salt = data.Salt;

            return dataManager.GetCreate().Insert<UserEx>(user, out strErrorMessage);
        }
    }
}
